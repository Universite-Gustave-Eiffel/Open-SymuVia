    ...
    boost::shared_ptr<Vehicule> pVehLeader, boost::shared_ptr<Vehicule> pVehFollower, double & dbTr, double & dbTa)
{
    ...
    {        
        ...
        try
        {
            ...
            // construction de la chaÃ®ne des paramÃ¨tres spÃ©cifiques
...
void ScriptedCarFollowing::SetModuleName(const std::string & moduleName, Reseau * pNetwork)
{
    ...
    catch( error_already_set )
    {
        ...
        pNetwork->log() << Logger::Info; // rebascule en mode INFO pour ne pas avoir Ã  reprendre tous les appels aux log en prÃ©cisant que c'est des INFO. Ã  supprimer si on reprend tous les appels au log.
    }
}
