...
{
    // Pas de procÃ©dure d'aggressivitÃ© particuliÃ¨re par dÃ©faut
    return;
}
    ...
    VoieMicro * pVoieCible, bool bForce, bool bRabattement, bool bInsertion)
{
    ...
    {
        if(pVehLeaderOrig)
            dbSi = pVehLeaderOrig->GetPos(1) - m_pVehicle->GetPos(1);   // Espacement avec le vÃ©hicule leader de la meme voie
        else
        {
            if(bForce)
            {					
    ...
    // sil les interdistances physiques aprÃ¨s changement de voie sont respectÃ©es
    if( bRabattement )
    {
    ...
    {        
        ...
        dbDemandeOrig = m_pVehicle->GetDiagFonda()->GetDebitMax();      // Cas des vÃ©hicules qui veulent s'insÃ©rer depuis un TripNode (arrÃªt de bus par ex. )
    ...
    {
        dbPi = 1 / m_pNetwork->GetTimeStep();			// autre changement de voie forcÃ©e (bus qui quitte son arrÃªt, changement de voie pour prise de direction)	
    ...
    else                                         
    {
        ...
        if(dbSf>0)
        {
            ...
            if(pVehLeader)
                pVehLeader->GetDiagFonda()->CalculVitEqDiagrOrig(dbVitVoieCible, dbVal1, dbVal2, true); // Utilisation du diagramme fondamental du vÃ©hicule leader de la voie cible
            else
        ...
        if(dbSi > 0)
        {
            ...
            if(pVehLeaderOrig)
                pVehLeaderOrig->GetDiagFonda()->CalculVitEqDiagrOrig(dbVitVoieOrig, dbVal1, dbVal2, true); // Utilisation du diagramme fondamental du vÃ©hicule candidat au changement de voie
            else
                m_pVehicle->GetDiagFonda()->CalculVitEqDiagrOrig(dbVitVoieOrig, dbVal1, dbVal2, true); // Utilisation du diagramme fondamental du vÃ©hicule candidat au changement de voie
        ...
        {		
            ...
            m_pVehicle->SetPi(true);
            m_pVehicle->SetPi(dbPi);
            if( fabs(dbVitVoieOrig - dbVitVoieCible) > 0.00001 )		// Pas de rabattement si vitesse origine est diffÃ©rente de la voie cible
                return false; 
        }
        else
        {			  
    ...
    else
    {
        ...
        // Test d'une nouvelle forme pour calcul le S de la voie initiale dans l'Ã©quation de Phi
        double dbPhiTmp = 0;
        ...
        if (bForce)
        {
            if (m_pVehicle->GetVoie(1)->IsChgtVoieObligatoire(m_pVehicle->GetType()))
            {
                ...
                // Ã  nettoyer Ã©ventuellement aprÃ¨s.
        ...
        {
            // EVOLUTION nÂ°70 - activation de l'algorithme des merging objects Ã©galement dans le cas des convergents d'insertion "Ã©changeurs"
    ...
    boost::shared_ptr<Vehicule> pVehLeader, boost::shared_ptr<Vehicule> pVehFollower, double & dbTr, double & dbTa)
{
    ...
    if(!pPointDeConvergence->IsFreeFlow())    // RÃ©gime congestionnÃ© en aval
    {        
        ...
        if( dbDemande2 < dbq2g)
        {
            ...
            pPointDeConvergence->SetRandNumbers(0);             // remise Ã  zÃ©ro du compteur de tirage
        }
        else
        {                    
            double dbProba = dbq2g * pPointDeConvergence->GetCvg()->GetReseau()->GetTimeStep();   // ProbabilitÃ© 'insertion
            ...
            {                      
                ...
                pPointDeConvergence->SetRandNumbers(pPointDeConvergence->GetRandNumbers()-1);                       // DÃ©crÃ©mentation du compteur des tirages

                if( dbRand < dbProba)
                {
                    bResult = true;       // SuccÃ¨s                       
                    break;
                }                        
            }                                        
        }
    }
    else                // RÃ©gime fluide en aval
    {
        pPointDeConvergence->SetRandNumbers(0);  // On est en rÃ©gime fluide, // remise Ã  zÃ©ro du compteur de tirage
        ...
        if(bCondAval)
        {
            // 1 - Etude du follower : si il sort gÃ¨ne t'il le vÃ©hicule en attente
            if(pVehFollower)
            {

                // Recherche du lien de sortie prÃ©cÃ©dent
                Tuyau           *pSortie;
                Voie            *pNextVoie;
                ...
                if( pTAmP->get_Type_amont() == 'D' )
                {
                    ...
                    if(pDvgt && pDvgt->m_LstTuyAv.size()>1 )
                    {
                        ...
                        // Le follower est-il amenÃ© Ã  sortir par ce tronÃ§on ?

                        // Recherche de la voie prÃ©cÃ©dente
                        pNextVoie = NULL;
                        for(int i=0; i<(int)pVehFollower->m_LstVoiesSvt.size(); i++)
                        {
                            if( pVehFollower->m_LstVoiesSvt[i] == pDvgt->m_LstTuyAm.front()->GetLstLanes()[0] )
                            {
                                ...
                                {
                                    // Calcul de la voie suivante et MAJ de la liste des voies du vÃ©hicule                                            
                        ...
                        if(pNextVoie)
                        {
                            if(pPointDeConvergence->GetCvg()->GetGiratoire() && pNextVoie->GetParent() == pSortie)   // le follower sort
                            {
                                ...
                                // Le vÃ©hicule sortant ne gÃ¨ne pas le flux d'insertion
                                {
                ...
                else
                {
                    ...
                    {
                        if(!pVehFollower->IsItineraire(pPointDeConvergence->m_pTAv))  // le vÃ©hicule ne gÃ¨ne pas
                        {                        
                            pVehFollower.reset();
                            bResult = true;
                        }
            ...
            // 2 - Si convergent multivoie, et si le vÃ©hicule qui est en attente doit s'insÃ©rer sur la voie externe
            // on regarde si un vÃ©hicule en amont sur la voie interne peut gÃ©ner l'insertion 
            pVehFollInt.reset();
            if( pTAmP->getNb_voies()>1 && pPointDeConvergence->m_pTAv->getNb_voies() > 1)   // Test si convergent multivoie
            {
                ...
                {
                    // Recherche du vÃ©hicule follower sur la voie interne
                    ...
                    if(pVehFollInt)
                    {
                        ...
                        {   // Le vÃ©hicule en attente est gÃ©nÃ© par le vÃ©hicule arrivant sur la voie interne                           
                        }
                        else // Le vÃ©hicule sur la voie interne ne gÃ¨ne pas le flux d'insertion
                        {
            ...
            dbtfbarre = 0;  // tronÃ§on non prioritaire
            if( pPointDeConvergence->GetCvg()->m_pGestionCapteur->GetNbVehTotal(pPointDeConvergence->GetCvg()->GetCapteur((TuyauMicro*)pVAmNP->GetParent()), j) == 0)     // Aucun vÃ©hicule vu par le capteur pendant T
            ...
            dbtmbarre = 0;  // tronÃ§on prioritaire
            ...
            // Recherche du premier vÃ©hicule sur le lien prioritaire
            ...
            // Un vÃ©hicule dans dlag ?

            // Moment de l'insertion du vÃ©hicule en attente si elle existe     
            ...
            {
                // le vÃ©hicule en attente arrive au point de convergence aprÃ¨s que le dÃ©lai de follow-up se soit Ã©coulÃ©
                // Interpolation de la position du follower au moment oÃ¹ le vÃ©hicule en attente passe le point de convergence                
                ...
                {                    
                    ...
                    if(pVehFollInt)
                    {
                        if(pVehFollInt->GetVit(1)>0)         // Si le vÃ©hicule est arrÃªtÃ© car il veu	t sortir (en fluide), il ne gÃ¨ne pas l'insertion                       
            ...
            {
                // le vÃ©hicule en attente arrive au point de convergence avant que le dÃ©lai de follow-up se soit Ã©coulÃ© (et celui_ci
                // s'Ã©coule avant la fin du pas de temps courant
                // dbts correspond Ã  l'instant dans le pas de temps pour lequel la condition aval est vÃ©rifiÃ©e
...
{
    // destruction du contexte le plus ancien le cas Ã©chÃ©ant
    if(m_pPrevContext)
    {
        delete m_pPrevContext;
    }

    // dÃ©calage du dernier contexte calculÃ©
...
void AbstractCarFollowing::Compute(double dbTimeStep, double dbInstant, CarFollowingContext * pContext)
{
    ...
    // calcul de la distance parcourue Ã  partir de la vitesse ou accÃ©lÃ©ration en fonction du type de loi de poursuite
    ...
    // Pour dÃ©tecter les cas oÃ¹ la loi de poursuite pose problÃ¨me (divisions par zero, etc...)
