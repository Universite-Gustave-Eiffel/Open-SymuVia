...
bool ParkingParameters::LoadFromXML(Reseau * pNetwork, DOMNode * pParkingNode, ParkingParameters * pParentParameters, Logger * pLogger)
{
    ...
    {
        // ces attributs ne sont prÃ©sents que pour le noeud de paramÃ¨trage gÃ©nÃ©ral (donc sans parent).
    ...
    {
        // du coup, dans le cas des enfants, on recopie la valeur du parent pour ne pas s'embÃªter Ã  faire des tests Ã  l'utilisation :
    ...
    // Lecture des variations temporelles des paramÃ¨tres :
    double dbDuree;
    XMLSize_t countj = pParkingNode->getChildNodes()->getLength();
    for (XMLSize_t j = 0; j<countj; j++)
    {
        // Chargement de la variante temporelle des paramÃ¨tres de stationnement
