...
// Historique: 16/09/2008 (C.BÃ©carie - Tinea)
//             CrÃ©ation
//================================================================
(
)
{  
    // Nettoyage des groupes de traversÃ©es
...
//================================================================
    void Giratoire::Init
//----------------------------------------------------------------
// Fonction  : Initialisation du giratoire (construction des tronÃ§ons
// de l'anneau)
// Version du: 20/11/2007
// Historique: 20/11/2007 (C.BÃ©carie - Tinea)
//             CrÃ©ation
...
{    
    ...
    std::deque<TuyauMicro*> LstTI;                  // Liste des tuyaux internes crÃ©Ã©s
    std::string strTmp;
    ...
    for(int i=0; i<(int)m_LstTAmAv.size();i++)
    {
        ...
        // CrÃ©ation du tuyau interne en aval		
        ...
        // Ajout Ã  la liste des tuyaux et des tuyaux micro du rÃ©seau
        ...
        // Ajout Ã  la liste des tuyaux internes du giratoire
    ...
    for(int i=0; i<(int)m_LstTAmAv.size();i++)
    {
        ...
        // Si tronÃ§on d'entrÃ©e
        if(pT->GetBriqueAval()==this)
        {
            // CrÃ©ation d'un convergent   
            ...
            // MAJ des tronÃ§ons internes et d'entrÃ©e
        ...
        else    // TronÃ§on de sortie du giratoire
        {
            // CrÃ©ation d'un divergent    
            ...
            // MAJ des tronÃ§ons internes et de sortie
...
//             entre le tuyau interne passÃ© en paramÃ¨ters et 
//             le tuyau aval pTAv
// Remarque : en effet, pas besoin de connaitre le tronÃ§on
//            d'entrÃ©e dans le giratoire.
...
//----------------------------------------------------------------
// Fonction  : Retourne la liste des tuyaux internes du giratoire
//             dÃ©finissant l'itinÃ©raire de pTAm Ã  pTAv
...
//----------------------------------------------------------------
// Fonction  : Retourne la liste des tuyaux internes du giratoire
//             dÃ©finissant l'itinÃ©raire d'une voie amont Ã  une voie aval
// Version du: 30/09/2008
// Historique: 30/09/2008 (C.BÃ©carie - Tinea)
//             CrÃ©ation
//================================================================
...
// Historique: 20/10/2008 (C.BÃ©carie - Tinea)
//             CrÃ©ation
//================================================================
(    
)
{
	...
	// VÃ©rification si tous les tronÃ§ons internes ont Ã©tÃ© segmentÃ©s
    ...
    // Construction des mouvements des convergents aux entrÃ©es du giratoire
    for(int i=0; i<(int)m_LstTInt.size();i++)
    {
        pT = m_LstTInt[i];
        if( pT->get_Type_amont() == 'C')
        {
            ...
            if(pCvgt)
            {
                for(size_t j=0; j < (size_t)m_nVoie; j++)
                {
                    ...
                    // Mouvements non prioritaires (branche d'entrÃ©e)
                    // OTK - 04/09/2012 - correction bug nÂ°13
    ...
    // Initialisation des voies prÃ©cÃ©dentes des voies internes du giratoire
    for(int i=0; i<(int)m_LstTInt.size();i++)
    {
        pT = m_LstTInt[i];
    ...
    // Ajout des traversÃ©es pour les vÃ©hicules s'insÃ©rant sur le tronÃ§on interne
    for(int i=0; i<(int)m_LstTAmAv.size();i++)
    {
        pT = m_LstTAmAv[i];

        if( pT->GetBriqueAval() == this) // TronÃ§on d'entrÃ©e du giratoire
        {
            // Pour chaque voie du tronÃ§on d'entrÃ©e et chaque voie interne du giratoire sauf la voie extÃ©rieure
            ...
            for(int iVoie = 0; iVoie < nbVoie; iVoie++)
            {
                ...
                // Pas de traversÃ©e si giratoire monovoie
                for(int iVoieDes = 1; iVoieDes < nbVoieInt; iVoieDes++)
                {
                    ...
                    // CrÃ©ation du groupe de traversÃ©e
                    ...
                    // Association du groupe aux voies d'entrÃ©e et de sortie et ajout a la liste
        ...
        {
            // Pour chaque voie du tronÃ§on interne Ã  l'exception de la voie extÃ©rieure
            int nbVoieInt = GetNbVoie();
            // Pas de traversÃ©e si giratoire monovoie
            for(int iVoieSource= 1; iVoieSource < nbVoieInt; iVoieSource++)
            {
                ...
                // Association du groupe Ã  la voie interne et au tuyau de sortie et ajout a la liste
    ...
    // Calcul une fois pour toutes des couts Ã  vides pour chaque mouvement du giratoire
    for(size_t i=0; i<m_LstTAmAv.size();i++)
    {
        ...
        if( pT->GetBriqueAval() == this) // TronÃ§on d'entrÃ©e du giratoire
        {
            ...
            {
                pTNext = m_LstTAmAv[j];
                if( pTNext->GetBriqueAval() != this) // TronÃ§on de sortie du giratoire
                {
                    ...
                    for (size_t iTypeVeh = 0; iTypeVeh < m_pReseau->m_LstTypesVehicule.size(); iTypeVeh++)
                    {
                        ...
                        {
                            // rmq. : on n'utilise pas les mÃ©thodes GetCoutAVide qui demande un type de vÃ©hicule
                            // Ã  cause des variantes qu'il peut y avoir sur les troncon non internes, qui ne nous concernent pas ici.
...
{
    // rmq. : ajout d'une pÃ©nalitÃ© additive pour l'insertion et une autre pour la sortie du giratoire
...
// Gestion des traversÃ©es des giratoires
void Giratoire::CalculTraversee(Vehicule *pVeh, double dbInstant, std::vector<int> & vehiculeIDs)
{
    if(m_bSimulationTraversees)
    {
        // On dÃ©tecte si le vÃ©hicule entre dans le giratoire au cours du pas de temps
        // pour gÃ©rer une Ã©ventuelle traversÃ©e d'entrÃ©e dans le giratoire. Approximation faite :
        // une traversÃ©e ne sera pas dÃ©tectÃ©e si le vÃ©hicule "saute" le tronÃ§on d'entrÃ©e (PDT trÃ¨s grand ou troncon trÃ¨s petit)
        bool bEntree = false;
        for(int i=0; i<(int)m_LstTAmAv.size();i++)
        {
            ...
            if( pT->GetBriqueAval() == this) // TronÃ§on d'entrÃ©e du giratoire
            {
                if(pVeh->GetLink(1) == pT) // le vÃ©hicule est sur le troncon d'entrÃ©e du gir au debut du PDT
                {
                    if(pVeh->GetLink(0) && pVeh->GetLink(0) != pT) // le vÃ©hicule n'est plus sur le tronÃ§on d'entrÃ©e du gir Ã  la fin du PDT
                    {
                        // il y a traversÃ©e d'entrÃ©e
                        bEntree = true;
                    }
                }
            }
        ...
        // dÃ©tection de la sortie (traversÃ©e de sortie)
        bool bSortie = false;
        for(int i=0; i<(int)m_LstTAmAv.size();i++)
        {
            TuyauMicro * pT = m_LstTAmAv[i];

            if( pT->GetBriqueAmont() == this) // TronÃ§on de sortie du giratoire
            {
                if(pVeh->GetLink(0) == pT) // le vÃ©hicule est sur le troncon de sortie du gir Ã  la fin du PDT
                {
                    if(pVeh->GetLink(1) && pVeh->GetLink(1)->GetBriqueParente() == this) // le vÃ©hicule Ã©tait au dÃ©but du pas de temps sur un troncon interne au giratoire
                    {
                        // il y a traversÃ©e de sortie
                        bSortie = true;
                    }
                }
            }
        ...
        // Gestion de la traversÃ©e d'entrÃ©e
        if(bEntree)
        {
            GrpPtsConflitTraversee * pGrpTra = NULL;

            // rÃ©cupÃ©ration de la voie d'entrÃ©e
            VoieMicro * pVoieEntree = pVeh->GetVoie(1);
            // rÃ©cupÃ©ration de la voie d'insertion sur le giratoire (un peu plus compliquÃ© ...)
            ...
            // pTuyauSortie peut Ãªtre NULL si aucun itinÃ©raire n'a Ã©tÃ© trouvÃ© pour le vÃ©hicule...
            if(pTuyauSortie)
            {
                ...
                // RÃ©cupÃ©ration du groupe de conflits correspondant
                for(size_t i = 0; i < m_LstGrpTraEntree.size(); i++)
                {
                ...
                if(pGrpTra)
                {
                    ...
                    for( std::deque<boost::shared_ptr<PtConflitTraversee> >::iterator itTra = pGrpTra->lstPtsConflitTraversee.begin(); itTra != pGrpTra->lstPtsConflitTraversee.end(); itTra++)
                    {
                        ...
                        {
                            // on positionne le vÃ©hicule en attente d'insertion
                            ...
                            // dans le casdes giratoires il suffit d'un vÃ©hicule prioritaire dÃ©tectÃ© sur une des voies internes
        ...
        } // fin de la gestion des la traversÃ©e d'entrÃ©e

        // Gestion de la traversÃ©e de sortie
        if(bSortie)
        {
            GrpPtsConflitTraversee * pGrpTra = NULL;
            // RÃ©cupÃ©ration du groupe de conflits correspondant
        ...
        } // fin de la gestion des la traversÃ©e de sortie
    }
}

// Gestion de l'insertion sur les giratoires Ã  plusieurs voies
...
{
    // Si le tirage a dÃ©ja Ã©tÃ© effectuÃ© pour ce vÃ©hicule et pour ce couple entree/sortie, on le rÃ©utilise.
    bool bFound = false;
    ...
    if(!bFound)
    {
        // si le couple origine / destination correspond Ã  un coefficient manuel, on l'utilise.
        bool bManual = false;

        for(size_t i = 0; i < m_LstCoeffsInsertion.size() && !bManual; i++)
        {
            if(m_LstCoeffsInsertion[i].pTuyauSortie == pTAv // mÃªme tuyau de sortie
                && m_LstCoeffsInsertion[i].pTuyauEntree == pVAm->GetParent() // mÃªme tuyau d'entrÃ©e
                && m_LstCoeffsInsertion[i].nVoieEntree == pVAm->GetNum()) // mÃªme voie d'entrÃ©e
            {
                // Tirage de la voie d'insertion choisie par le vÃ©hicule
        ...
        if(!bManual)
        {
            ...
            // boucle sur l'ensemble des troncons extÃ©rieurs du giratoire pour compter
            // le nombre de sorties, et l'index de la sortie pTAv par rapport Ã  l'entrÃ©e pVAm
            ...
            // Construction pour chaque voie du giratoire du coefficient adaptÃ©
            for(int iVoie = 0; iVoie < m_nVoie; iVoie++)
            {
                boost::shared_ptr<MouvementsSortie> mvt(new MouvementsSortie());
                mvt->m_pReseau = m_pReseau;
            ...
            else
            {
                ...
                {
                    // Cas oÃ¹ un tirage est nÃ©cessaire
        ...
        // Ajout des mouvements autorisÃ©s pour le vÃ©hicule en question
...
// Ajout de coefficients manuels (prioritaires sur les coefficients calculÃ©s par dÃ©faut)
...
// coordonnÃ©es des points de l'anneau interne.
...
)
{
    ...
    // Parcours des tronÃ§ons internes
