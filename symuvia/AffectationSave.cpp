...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// M̩thode(s) de la classe "CItineraire" qui g̬re la sauvegarde des itin̩raires pr̩dits
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// M̩thode(s) de la classe "CIteration" qui g̬re la sauvegarde des it̩rations pr̩dites
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// M̩thode(s) de la classe "CCoupleOD" qui g̬re la sauvegarde des couples Origine/Destination
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

CIteration * CCoupleOD::AddPreditIteration(size_t num)
{
	// Calcul de l'instant de d̩but
	STimeSpan TS;
	STime temps_debut_calcul;
	size_t count = predit.size();
	if ( count == 0 )//Liste vide, il faut cr̩er une it̩ration
	{
		CIteration iter;
		// Initialisation calcul de la dur̩e du calcul
		iter.temps_debut_calcul = SDateTime::Now().ToTime();
		iter.temps_calcul = 0;
		// Positionnement de valeurs
		iter.num = num;
		iter.indice_proximite = -1;
		// Ajout de l'it̩ration
	...
	{	// Temps de fin du dernier
		...
		if (num <= lastNum) // On est tjs dans la m̻me it̩ration mais avec un autre itin̩raire
		{					// Il faut modifier l'it̩ration pour l'ajout d'un itin̩raire
							// En fait (num == lastNum)
			...
			// Renvoyer l'adresse de l'it̩ration courante
			return pIter;
		}
		else // Il faut effectivement ajouter une it̩ration (nouvelle it̩ration)
		{
			CIteration iter;
			// Initialisation calcul de la dur̩e du calcul
			iter.temps_debut_calcul = SDateTime::Now().ToTime();
			...
			// Ajout de l'it̩ration
			predit.push_back(iter);
			// Renvoyer l'adresse de la derni̬re it̩ration de la liste (celle qu'on vient d'ajouter)
			return &predit[count];
		}
	}
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// M̩thode(s) de la classe "CPeriode" qui g̬re la sauvegarde d'une p̩riode d'affectation
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CPeriode::FinPeriode(SymuCoreManager * pSymuScript, const std::string & sType, int nb_iterationA, double indicateur_proximiteA, double finA, double seuil_tempsparcoursA, bool bForceGraphReset)
{
	...
	// Calcul des temps de parcours r̩alis̩	
...
{
	// Nettoyage des coefficients pr̩dits et r̩alis̩s
...
void CPeriode::SetLastPreditIterationIndProx(double dbIndProx)
{
	...
	for (it=periode_type_vehicule.begin(); it!=itEnd; it++)//it̩ration sur les types de v̩hicule
	{
		std::map<std::pair<std::string, std::string>, CCoupleOD>::iterator it2, itEnd2;
		itEnd2 = it->second.end();
		for (it2=it->second.begin(); it2!=itEnd2; it2++)//it̩ration sur les couples OD
		{
			CCoupleOD * pCplOD = &it2->second;
			std::deque<CIteration>::iterator it3, itEnd3;
			itEnd3 = pCplOD->predit.end();
			for (it3 = pCplOD->predit.begin(); it3!=itEnd3; it3++)//it̩ration sur les "CIt̩ration"
			{
