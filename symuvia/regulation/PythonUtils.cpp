...
BOOST_PYTHON_MODULE(symuvia)
{
    ...
    // Wrapper pour la liste des vÃ©hicules du rÃ©seau
    ...
    // Wrapper pour la liste des lignes de bus du rÃ©seau
    ...
    // Wrapper pour la liste des arrÃªts de bus d'une ligne
    ...
    // wrapper pour le container des coefficients d'un mouvement autorisÃ©
...
void PythonUtils::initialize()
{
    ...
    if(!Py_IsInitialized())
    {
        // Cas classique : SymuVia n'est pas lancÃ© depuis python
        Py_Initialize();
    }
    else
    {
        // Cas oÃ¹ SymuVia est lancÃ© depuis python
        PyEval_AcquireLock();
    ...
    try
    {
        // rÃ©cupÃ©ration du contexte python global
    ...
    catch( error_already_set )
    {
        if (m_pLogger)
        {
            ...
            *m_pLogger << Logger::Info; // rebascule en mode INFO pour ne pas avoir Ã  reprendre tous les appels aux log en prÃ©cisant que c'est des INFO. Ã  supprimer si on reprend tous les appels au log.
        }
        else
        {
...
void PythonUtils::buildDictFromNode(DOMNode * pNode, dict &rDict)
{
    ...
    for(XMLSize_t iChild = 0; iChild < pNode->getChildNodes()->getLength(); iChild++)
    {
        ...
        if(pChild->getNodeType() == DOMNode::ELEMENT_NODE)
        {
            ...
            // Si pas encore d'Ã©lÃ©ment de ce nom dans le dict, on crÃ©e la liste et on ajoute l'Ã©lÃ©ment Ã  la liste
...
void PythonUtils::buildNodeFromDict(DOMElement* pElement, const boost::python::dict & rDict)
{
    ...
    {
        // Si l'Ã©lÃ©ment fils est un dictionnaire, on crÃ©e un noeud fils correspondant Ã  ce dictionnaire
        ...
        {
            // Sinon, on ajoute l'Ã©lÃ©ment fils en tant qu'attribut
