...
void SimulationSnapshot::SwitchToTempFiles(Reseau * pReseau, size_t snapshotIdx)
{
    ...
    // OTK - rmq. vu ce code, l'affectation dynamique convergente ne doit pas marcher si on a plusieurs plages temporelles... on aura au mieux une fuite mÃ©moire. A reprendre ?
    ...
    // on passe en mode "Ã©criture dans fichiers temporaires" pour les sorties CSV
...
void SimulationSnapshot::ValidateTempFiles(Reseau * pReseau)
{
    ...
    //Recopie des instants de la pÃ©riode d'affectation dans le fichier trafic
    std::deque< TimeVariation<TraceDocTrafic> >::iterator it;
    for (it = pReseau->m_xmlDocTrafics.begin(); it != pReseau->m_xmlDocTrafics.end(); it++)
    {
...
void SimulationSnapshot::Backup(Reseau *pReseau)
{
	...
	// Sauvegarde de l'instant simulÃ©
    ...
    // Les contrÃ´leurs de feux (pour les LGP)
	...
	for( itT = pReseau->m_LstTuyaux.begin(); itT != pReseau->m_LstTuyaux.end(); itT++)
	{
        ...
        // rmq. : comme CTuyauMeso hÃ©rite de TuyauMicro, les variables de simulation de la classe TuyauMicro
        // sont dÃ©jÃ  sauvegardÃ©es par le bloc ci-dessus.
	...
	for( itE = pReseau->Liste_origines.begin(); itE != pReseau->Liste_origines.end(); itE++)
	{
		...
		// Les vÃ©hicules en attente aux entrÃ©es...
	...
	{
        // sauvegarde des donnÃ©es capteurs
        ...
        // sauvegarde des numÃ©ros de pÃ©riodes
        m_nGestionCapteurPeriodes = pReseau->GetGestionsCapteur()->GetPeriodes();
	}

    // sauvegarde de l'Ã©tat des calculs de temps de parcours pour la sortie JSON
    if (m_pTravelTimesOutputSnapshot)
    {
        delete m_pTravelTimesOutputSnapshot;
        m_pTravelTimesOutputSnapshot = NULL;
...
void SimulationSnapshot::Restore(Reseau *pReseau)
{
	...
	// Suppression des noeuds vÃ©hicules du fichier de sortie
	...
	// Restitution des vÃ©hicules Ã  partir de la liste sauvegardÃ©e
    ...
    // Restitution des leaders des vÃ©hicules prÃ©sents sur le rÃ©seau (les pointeurs ici prÃ©sents ne sont plus valables car
    // les vÃ©hicules ont Ã©tÃ© rÃ©instanciÃ©s)
    std::vector<boost::shared_ptr<Vehicule>>::iterator itVeh;    
	for(itVeh = pReseau->m_LstVehicles.begin(); itVeh != pReseau->m_LstVehicles.end(); ++itVeh)
	{
    ...
    // Restitution de l'Ã©tat des contrÃ´leurs de feux
	std::deque<ControleurDeFeux*>::iterator itCDF;
    for( itCDF = pReseau->Liste_ControleursDeFeux.begin(); itCDF != pReseau->Liste_ControleursDeFeux.end(); itCDF++)
	{
	...
	// Restitution des donnÃ©es de simulation d'un tronÃ§on et de ses voies
	std::deque<Tuyau*>::iterator itT;
	for( itT = pReseau->m_LstTuyaux.begin(); itT != pReseau->m_LstTuyaux.end(); itT++)
	{
        TuyauMicro * pTuyauMicro = dynamic_cast<TuyauMicro*>(*itT);
        if(pTuyauMicro)
        {
            ...
            // On restitue la rÃ©solution du tuyau et on place le tuyau dans la bonne liste le cas Ã©chÃ©ant
    ...
    // Restitution de l'Ã©tat des zones
    for (size_t iZone = 0; iZone < pReseau->Liste_zones.size(); iZone++)
    {
        ZoneDeTerminaison * pZone = pReseau->Liste_zones[iZone];
	...
	// Restitution de l'Ã©tat des entrÃ©es
	std::deque<SymuViaTripNode*>::iterator itE;
	for( itE = pReseau->Liste_origines.begin(); itE != pReseau->Liste_origines.end(); itE++)
	{
    ...
    // Restitution de l'Ã©tat des flottes de vÃ©hicules (aprÃ¨s la restitution des vÃ©hicules en attente)
    for (size_t iFleet = 0; iFleet < pReseau->m_LstFleets.size(); iFleet++)
    {
        AbstractFleet * pFleet = pReseau->m_LstFleets[iFleet];
	...
	if (pReseau->GetGestionsCapteur() != NULL)
	{
        ...
        // restitution des donnÃ©es capteurs
        const std::vector<AbstractSensor*> & sensors = pReseau->GetGestionsCapteur()->GetAllSensors();
        for(size_t iSensor = 0; iSensor < sensors.size(); iSensor++)
        {
            AbstractSensor * pSensor = sensors[iSensor];
    ...
    // Restitution de l'Ã©tat du gestionnaire de sortie des temps de parcours
    if (pReseau->GetTravelTimesOutputManager())
    {
...
{
    // rmq. : pas la peine de sÃ©rialiser pour l'instant, car utilisÃ© uniquement pour SymuMaster
}
