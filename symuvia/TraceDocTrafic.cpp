...
void TraceDocTrafic::AddVehicule(int nID, const std::string & strLibelle, const std::string & strType, const std::string & strGMLType, double dbKx, double dbVx, double dbW, const std::string & strEntree, const std::string & strSortie, const std::string & strZoneEntree, const std::string & strZoneSortie, const std::string & strRoute, double dbInstCreation, const std::string & sVoie, bool bAgressif, int iLane, const std::string & sLine, const std::vector<std::string> & initialPath, const std::deque<PlageAcceleration> & plagesAcc, const std::string & motifOrigine, const std::string & motifDestination, bool bAddToCreationNode)
{
    for(size_t iDoc = 0; iDoc < m_LstDocTrafic.size(); iDoc++)
    {
        m_LstDocTrafic[iDoc]->AddVehicule(nID, strLibelle, strType, strGMLType, dbKx, dbVx, dbW, strEntree, strSortie, strZoneEntree, strZoneSortie, strRoute, dbInstCreation, sVoie, bAgressif, iLane, sLine, initialPath, plagesAcc, motifOrigine, motifDestination, bAddToCreationNode);
    }
}
...
void TraceDocTrafic::UpdateInstSortieVehicule(int nID, double dbInstSortie, const std::string & sSortie, double dstParcourue, const std::vector<Tuyau*> & itinerary, const std::map<std::string, std::string> & additionalAttributes)
{
    for(size_t iDoc = 0; iDoc < m_LstDocTrafic.size(); iDoc++)
    {
        m_LstDocTrafic[iDoc]->UpdateInstSortieVehicule(nID, dbInstSortie, sSortie, dstParcourue, itinerary, additionalAttributes);
    }
}
