...
{
    // On vÃ©rifie que ce couple n'a pas dÃ©jÃ  Ã©tÃ© ajoutÃ©
...
/**
 * Calcule la couleur des feux Ã  l'instant prÃ©cisÃ© et retourne vrai si le feu est
...
{    
    eCouleurFeux eCouleur;          // Indique la couleur de feux Ã  l'instant considÃ©rÃ©
    eCouleurFeux ePrevCouleur;      // Indique la couleur de feux Ã  l'instant considÃ©rÃ©
    ...
    // Ajout du couple entrÃ©e sortie s'il n'est pas dÃ©jÃ  recensÃ©
    ...
    if((eCouleur == FEU_VERT) && (ePrevCouleur == FEU_ROUGE))     // Rouge au dÃ©but du pas de temps et vert Ã  la fin
    {
        return FEU_VERT;
    }

    if((eCouleur == FEU_ROUGE) && ((ePrevCouleur == FEU_VERT) || (ePrevCouleur == FEU_ORANGE)))     // Vert (ou orange) au dÃ©but du pas de temps et rouge Ã  la fin
    {
		if (pdbPropVertOrange != NULL)
			*pdbPropVertOrange = 1;
...
(
	...
	STimeSpan   &tsTpsEcPlan    // Retourne le temps Ã©coulÃ© depuis le dÃ©but du plan
)
{
    ...
    {        
        ...
        if(pPDF)
        {
            if( pPDF->GetStartTime() > tHrCrt)  // si l'heure de dÃ©but du premier plan est supÃ©rieur Ã  l'instant traitÃ©
            {
        ...
        // Calcul de la durÃ©e Ã©coulÃ©e depuis le dÃ©but du plan de feux
...
(   
    double dbInstant,     
    double  &dbTpsPrevSeqs,     // Somme des durÃ©es des sÃ©quences prÃ©cÃ©dentes du cyle
    double  &dbTpsCurSeq,       // Temps depuis le dÃ©but de la sÃ©quence courante
    bool    &bPrioritaire       // sÃ©quence liÃ©e Ã  un VGP ?
)
{
    PlanDeFeux* pPDF;
    ...
    if(!m_PDFActif)   
    {
        ...
        // Le plan de feux calculÃ© est-il remplacÃ© par un plan de feux de VGP prioritaire
        if(m_pPDFEx && dbInstant >= m_dbDebPDFEx)
        {
            ...
            {
                m_pPDFEx.reset();    // le plan de feux de VGP prioritaire est terminÃ©
            }
        }
    }
    else
    ...
    // Recherche de la sÃ©quence concernÃ©e
...
{    
    ...
    // Recherche du signal actif concernÃ©
...
// Mise Ã  jour des caractÃ©ristiques dymamiques du contrÃ´leur de feux Ã  la fin du pas de temps considÃ©rÃ©
void ControleurDeFeux::Update(double dbInstant)
{
    bool bPDFChange = false;

    // Un VGP a t'il Ã©tÃ© vu par un capteur amont ?
    for(int i=0; i< (int)m_LstLGP.size(); i++)
    {
        ...
        for(int j=0; j < (int)plgp->LstLTG.size(); j++)
        {
            ...
            for(int k=0; k< (int)lstBus.size(); k++)
            {
                ...
                {                        
                    // Mise en place du plan de feux 'pre-prioritÃ©'

                    // Calcul de la durÃ©e pour que le VGP atteigne la zone controlÃ©e
                    double dbTZone, dbDst;                        
                    double dbDstArret;
                    TripNode  *objArr;
                    double dbPasTemps = m_pReseau->GetTimeStep();

                    // Existe t'il un arrÃªt entre la postion du VGP et l'entrÃ©e de la zone ?
                    // On fait l'hypothÃ¨se qu'il existe au plus un arrÃªt
                    dbDstArret = pBus->GetNextTripNodeDistance(0, &objArr);

                    // Distance qui reste Ã  parcourir avant l'entrÃ©e de la zone du contrÃ´leur
                    double dbPosEntreeZone = plgp->dbPosEntreeZone;
                    // Cas oÃ¹ le tronÃ§on du bus est avant le tronÃ§on de la zone : on se remet dans le repÃ¨re du tronÃ§on du bus.
                    // rmq. ne va pas fonctionner si plus d'un tronÃ§on entre le capteur amont et la zone (pas grave ?)
                    ...
                    if(dbDstArret>=dbDst)    // Pas d'arrÃªt Ã  gÃ©rer
                    {
                    ...
                    else // Un arrÃªt Ã  gÃ©rer
                    {                                       
                        dbTZone = pBus->CalculTempsAvecArret(dbInstant, dbPasTemps, 0, plgp->pTEntreeZone, plgp->dbPosEntreeZone, objArr);
                    }

                    // Si on avait dÃ©jÃ  un vÃ©hicule guidÃ© prioritaire, une sÃ©quence a dÃ©jÃ  Ã©tÃ© Ã©tablie pour le faire passer.
                    // On ne va donc que rajouter un signal pour le nouveau vÃ©hicule guidÃ© prioritaire, en prolongeant la sÃ©quence si nÃ©cessaire
                    bool bFirstVGP = m_LstVGPs.size() == 0;

                    // Ajout du VGP et de la LGP Ã  la liste Ã  traiter
                    m_LstVGPs.push_back(make_pair(pBus, plgp));

                    // RecupÃ©ration des troncons d'entrÃ©e et de sortie du CAF
                    ...
                    if(bFirstVGP)
                    {
                        ...
                        // Dans le cas oÃ¹ on Ã©tait en mode Post prioritÃ© pour des VGPs prÃ©cÃ©dent, on a dÃ©jÃ  un plan de feux particulier en place : on le termine
                        if(m_pPDFEx)
                        {
                            m_pPDFEx.reset();
                        }

                        // Calcul de l'instant Ã  partir duquel il faut appliquer le plan prioritÃ©                        
                        ...
                        CDFSequence* pSeqZone = GetSequence(dbInstant + dbTZone, dbTpsPrevSeqs, dbTpsCurSeq, bPrioritaire);  // SÃ©quence d'arrivÃ©e
                        ...
                        // Calcul de la sÃ©quence actuelle
                        CDFSequence* pActSequence = GetSequence(dbInstant, dbTpsPrevSeqs, dbTpsCurSeq, bPrioritaire);                        

                        // CrÃ©ation du plan de feux de la phase du VGP prioritaire                        
                        m_pPDFEx.reset(new PlanDeFeux());
                        if( dbInstant <= dbInstPlanPrePriorite)
                            m_dbDebPDFEx = dbInstPlanPrePriorite; // DÃ©but du plan d'application plus tard
                        else 
                            m_dbDebPDFEx = dbInstant;   // le plan est appliquÃ© dÃ¨s maintenant
                        m_dbFinPDFEx = m_dbDebPDFEx + DBL_MAX;

                        // Couple de sÃ©quences de retour (aprÃ¨s la sortie du VGP
                        m_pSeqRetour1 = pPDF->GetNextSequence( pActSequence );
                        m_pSeqRetour2 = pPDF->GetNextSequence( m_pSeqRetour1 );

                        // DÃ©termination du type de gestion du mode 'pre-prioritÃ©'

                        // Calcul de la durÃ©e minimale pour la sÃ©quence d'arrivÃ©e
                        ...
                        if((pSeqZone == pActSequence) && (dbTpsCurSeq +  dbTZone  >=  dbDureeMin + m_dbDureeRougeDegagement))
                        {
                            ...
                            // Construction de la sÃ©quence Ã  appliquer en prÃ©-prioritÃ©                            
                            // Construction des sÃ©quences modifiÃ©es
                            CDFSequence *pSeq;
                            ...
                            if(!plgp->bPrioriteTotale && plgp->nSeqPartagee == pSeqZone->GetNum() ) // Prolongment de la sÃ©quence
                            {
                            ...
                            else
                            {
                                ...
                                pSeq = new CDFSequence();
                                pSeq->SetNum(1);
                                pSeq->SetTotalLength ( DBL_MAX);  // DurÃ©e de la sÃ©quence de prioritÃ© totale infinie
                                m_pPDFEx->AddSequence(pSeq);
                                SignalActif *pSA = new SignalActif(pTuyauAmont, pTuyauAval, pSeq->GetTotalLength(), 0, 0);
                                pSeq->AddActiveSignal(pSA);
                            }
                        ...
                        {
                            if(pSeqZone == pActSequence) // la sÃ©quence d'arrivÃ©e du VGP dans la zone de contrÃ´le est Ã©gale Ã  la sÃ©quence actuelle
                            {
                                m_nTypeGestionPrePriorite = 2;  // 'sÃ©q. actuelle Ã©courtÃ©e'
                                m_pSeqPrePriorite = pSeqZone;
                                if(m_pReseau->IsVGPDebug())
                                ...
                                pSeq = new CDFSequence();
                                pSeq->Copy( pSeqZone );
                                double dbDureeMin = pSeq->GetMinDuree(m_dbDureeVertMin);    // DurÃ©e de vert min
                                ...
                                // SÃ©quence de rouge de dÃ©gagement
                                pSeq = new CDFSequence();
                                pSeq->Copy( pSeqZone );
                                pSeq->SetNum(1);
                            ...
                            {
                                m_nTypeGestionPrePriorite = 3;  // 'prolongement sÃ©q. actuelle'
                                ...
                                // PrioritÃ©
                                pSeq = new CDFSequence();
                                // si prioritÃ© partagÃ©e
                                if(!plgp->bPrioriteTotale && plgp->nSeqPartagee == pSeqZone->GetNum() ) // ajout des prioritÃ©s partagÃ©es
                                {
                                    pSeq->Copy(pSeqZone);
                                }
                                pSeq->SetNum(1);
                    ...
                    {
                        // Ici, on est sensÃ© avoir un plan de feu pour assurer la prioritÃ© des VGP, et on n'est pas encore en mode "sÃ©quences de retour Ã  la normale"
                        assert(m_cModeFct != 'Q');
                        assert(m_pPDFEx);
                        // Dans ce cas, c'est facile, en supposant qu'il n'y a pas de croisement entre les lignes prioritaires,
                        // on ajoute simplement Ã  la derniÃ¨re sÃ©quence de durÃ©e infinie (qui se terminera Ã  la dÃ©tection de la sortie du dernier VGP en cours de traitement)
                        // le signal actif permettant de faire circuler le vÃ©hicule qui arrive.
    ...
    if( m_cModeFct == 'O' ) 
    {
        // Un VGP en cours de traitement est-il entrÃ© dans la zone du contrÃ´leur de feux
        for(size_t i = 0; i < m_LstVGPs.size(); i++)
        {
            ...
            {
                // Modification du mode de fonctionnement : si un VGP est entrÃ© dans la zone de contrÃ´le, on passe en mode 'prioritÃ©'
                m_cModeFct = 'P';
                break;
            }
        }
    ...
    if( m_cModeFct == 'P' ) 
    {
        ...
        {
            // Suppression des vÃ©hicules qui sont sortis de la zone
            if( iter->first->IsPasse( iter->second->pTCptAv, iter->second->dbPosCptAv ) || !iter->first || !iter->first->GetLink(0) ) // cas du bus NULL : le bus est sorti du rÃ©seau (dernier arrÃªt atteint par exemple)
            {
        ...
        // Une fois tous les vÃ©hicules prioritaires sortis de la zone, on passe en mode post-prioritÃ©
        if( m_LstVGPs.size() == 0 )
        {
            // Modification du mode de fonctionnement : si un VGP est sorti de la zone de contrÃ´le, on passe en mode 'post-prioritÃ©'
            m_cModeFct = 'Q';

            // Mise en place du plan de feux 'post-prioritÃ©'
            if(m_pPDFEx)
                m_pPDFEx.reset();
 
            // Construction du couple de sÃ©quence pour redevenir Ã  la normal

            // Recherche de la sÃ©quence de retour 2 dans la suite des sÃ©quences normales            
            double dbDureeRetour; // DurÃ©e du retour Ã  la normal
            ...
            // Dilation ou rÃ©duction du couple de sÃ©quence de retour
            m_pPDFEx.reset(new PlanDeFeux);
    ...
    if (bPDFChange)
    {
        // Gestion pour le mode mÃ©so de l'Ã©vÃ¨nement de sortie du mdoe prioritaire du CDF :
        // les arrivals sur les tuyaux amont du noeud dont l'instant correspond Ã  la fin de la simu
        // correspondent Ã  des arrivÃ©es au moment du cycle de feu prioritaire de durÃ©e indÃ©terminÃ©e.
        // on recalcule donc les instants d'arrivÃ©es en prenant l'instant de vert suivant qu'on peut Ã  prÃ©sent calculer :

        // rÃ©cupÃ©ration des noeuds liÃ©s Ã  ce CDF :
                ...
                for (size_t iUpstreamLink = 0; iUpstreamLink < pNode->m_LstTuyAssAm.size(); iUpstreamLink++)
                {
                    ...
                    for (std::list<std::pair<double, Vehicule*> >::iterator iterArrival = arrivals.begin(); iterArrival != arrivals.end(); ++iterArrival)
                    {
                        if (iterArrival->first >= m_pReseau->GetDureeSimu())
                        {
                            ...
                            assert(pOutgoingLink != NULL); // Si ca arrive, prÃ©voir un recalcul du tuyau suivant ici...

                            double dbNextGreen = GetInstantVertSuivant(dbInstant, m_pReseau->GetDureeSimu(), pUpstreamLink, pOutgoingLink);

                            // rmq. : petit Ã©cart au modÃ¨le mÃ©so ici le next supply time du tuyau aval...
                    ...
                    // dans le cas oÃ¹ le plus petit arrivalTime est infÃ©rieur au next arrival time courant, on remplace le next arrival par le minArrival
...
// Historique: 20/04/2009 (C.BÃ©carie - Tinea)
//================================================================
(   
	const std::string & sSP     // ID du plan de feux Ã  activer
)
{
    PlanDeFeux *pPDF;
    ...
    if(!pPDF)
        return -3;  // Plan de feux non dÃ©finie
...
//----------------------------------------------------------------
// Fonction  : DÃ©termine les tuyaux amont et aval du CAF controlÃ©
//             par lequel va arriver le vÃ©hicule guidÃ© prioritaire
//             passÃ© en paramÃ¨tre
// Remarque  : Dans le cas oÃ¹ le CDF controle plusieurs
//             connexions, on fait l'hypothÃ¨se que celui qui
//             concerne le calcul actuel est le prochain qui est
//             traversÃ© par le vÃ©hicule guidÃ© prioritaire
// Version du: 09/09/2011
// Historique: 09/09/2011 (O.Tonck - IPSIS)
//================================================================
(   
	Vehicule * pBus,         // vÃ©hicule en approche des feux
    Tuyau ** pTuyAmont,
    Tuyau ** pTuyAval
)
{
    ...
    *pTuyAval = NULL;
    ...
    // rmq. problÃ¨me potentiel ici si tuyau(0) et tuyau(1) sont tous les deux internes. regarder les tuyaux parents des voies parcourues si c'est le cas ?
    ...
    if(tuyIdx != -1)
    {
        ...
        // tant que la connexion aval du tuyau n'est pas controlÃ©e par this,
        // on avance dans l'itinÃ©raire
...
double ControleurDeFeux::GetInstantVertSuivant(double dRefTime, double dInstantMax, Tuyau * pTuyauAmont, Tuyau * pTuyauAval )
{
    ...
    // rmq. : en cas de ligne guidÃ©e prioritaire et en mode meso, on ne dÃ©tecte pas avec cette mÃ©thode le prochain instant de vert... du coup
    // on recalcule les prochains instants d'arrivÃ©e meso en fin de mÃ©thode ::Update lors des modifications dynamique des plans de feux Ã  cause
    // des lignes guidÃ©es prioritaires.
    ...
    do
    {
        ...
        // passage Ã  la sÃ©quence suivante
        dInstantSequence += pSequence->GetTotalLength() - dTpsCurSeq;

        // Pour ne pas boucler jusqu'Ã  la fin de la simulation si un feu est toujours rouge par exemple
        if(!pSignal)
        {
...
double ControleurDeFeux::GetInstantDebutVert(double dRefTime, Tuyau *pTuyauAmont, Tuyau * pTuyauAval)
{
    ...
    dbInstantDebutVert = (int)dRefTime - dTpsCurSeq; // rmq. : on tronque dRefTime car dTpsCurSeq est tronquÃ© dans GetSequence ce qui produit un rÃ©sultat incohÃ©rent dans certains cas

    SignalActif *pSignal = pSequence->GetSignalActif(pTuyauAmont, pTuyauAval);

    assert(pSignal); // Si on est lÃ  c'est que le feu est vert : on doit avoir le signal ...
...
double ControleurDeFeux::GetMeanWaitTime(Tuyau *pTuyauAmont)
{
    ...
    else
    {
        ...
        if(pPDF)
        {
            // Pour chaque tronÃ§on aval possible, calcul du temps d'attente moyen, puis on moyenne les temps d'attente pour sa ramener Ã  une unique
            // valeur pour le tronÃ§on amont
            ...
            {
                // On dÃ©termine les plages de feux non verts pour le mouvement considÃ©rÃ© (en fusionnant les durÃ©es de rouge le cas Ã©chÃ©ant).
                // Pas la peine de s'occuper des moments ou le feu est vert.
                // On se cale pour notre t0 sur une dÃ©but de feu vert pour ne pas avoir Ã  gÃ©rer le bouclage du cycle.
                std::vector<double> redPeriods;
                double dbCurrentRedLength = 0.0;
                double dbDelayBeforeFirstGreen = 0.0;
                bool bFirstGreen = false;
                // Parcours des sÃ©quences du plan de feux.
                for (size_t i = 0; i < pPDF->GetLstSequences().size(); i++)
                {
                    CDFSequence * pSeq = pPDF->GetLstSequences()[i];
                    ...
                    {
                        // le feu est rouge sur toute la durÃ©e de la sÃ©quence
                        if (!bFirstGreen)
                        {
                    ...
                    {
                        // le feu passe au vert au moins une partie de la sÃ©quence
                        if (!bFirstGreen)
                        {
                        ...
                        else
                        {
                            ...
                            // rÃ©initialisation pour la prochaine sÃ©quence de rouge
                            dbCurrentRedLength = pSeq->GetTotalLength() - pSignal->GetActivationDelay() - pSignal->GetGreenDuration();
                        }
                    }
                }
                // On ajoute la derniÃ¨re sÃ©quence
                ...
                // On divise par la durÃ©e totale sur 2
            ...
            } // Fin boucle sur les tronÃ§on aval
...
(
    double dbInstCycle,     // Instant considÃ©rÃ© par rapport au dÃ©but du cycle courant
    double &dbTpsPrevSeqs,  // Temps passÃ© dans les sÃ©quenes prÃ©cÃ©dentes du cycle courant
    double &dbTpsCurSeq     // Temps passÃ© dans la sÃ©quence courante
)
{
    dbTpsPrevSeqs = 0;
...
// Retourne la somme des durÃ©es cumulÃ©es des nNum premiÃ¨re sÃ©quences
...
// Retourne la durÃ©e minimum d'une sÃ©quence afin que pour chaque signal l'allumage et un temps de vert supÃ©rieur Ã  la durÃ©e de vert minimum soit respectÃ©
...
// Retourne la plus petite durÃ©e de vert sur l'ensemble des signaux actifs
...
double CDFSequence::GetInstantVertSuivant(double dInstant, Tuyau *pTuyauAmont, Tuyau * pTuyauAval )
{
    ...
    for(int nI = (int)floor(dInstant); nI < (int)m_dbDureeTotale; nI++)
    {
        ...
        if(nNbVertTmp < nNbVert &&   // infÃ©rieur Ã  la valeur stockÃ©
             dInstant< nI) // sup Ã  l'intanst voulu
        {
            nNbVert = nNbVertTmp;
...
// associÃ© Ã  un couple d'entrÃ©e/sortie
...
// Calcul la proportion de pas de temps pour laquelle le feu a Ã©tÃ© vert ou orange
