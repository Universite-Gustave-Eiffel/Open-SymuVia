    ...
    GMLDocTrafic * pOriginalDoc, bool bHasAtLeastOneCDF, bool bHasSensor)
{
    ...
    // *********************************************
    // Si enveloppe vide : cas des documents temporaires pour affectation dynamique convergente : on ne recrÃ©e pas le fichier _net qui n'aura pas changÃ© par rapport Ã  celui dÃ©jÃ  Ã©crit
    if(!bTmpDoc)
    {
	    // XmlWriter
    ...
    // Fichier pour les trajectoires, dont le contenu sera injectÃ© dans le fichier complet Ã  la fin
    ...
    m_XmlWriterTrajsTemp->writeAttributeString(XS("type"), XS("xsd:double"));
    m_XmlWriterTrajsTemp->writeStartElement(XS("mf:AttrAnnotation"));
    m_XmlWriterTrajsTemp->writeText(XS("Vehicle acceleration in m/sÂ² during the trajectory"));
    m_XmlWriterTrajsTemp->writeEndElement(); // mf:AttrAnnotation
    m_XmlWriterTrajsTemp->writeEndElement(); // mf:attrDef
    ...
    if(bHasSensor)
    {
        ...
        // Ajout de la dÃ©finition du capteur global
...
{
    // Le fichier rÃ©seau est terminÃ© dÃ¨s le dÃ©but, on le termine donc dÃ¨s le PostInit
...
void GMLDocTrafic::Terminate()
{
    if(m_XmlWriterTrafficLights!=NULL)
    {
        // terminaison du fichier des Ã©tats des feux
        if(!m_bReinitialized)
        {
    ...
    if(m_XmlWriterTrajsHeader!=NULL)
    {
        // Terminaison du fichier dÃ©finitif des trajectoires
        m_XmlWriterTrajsHeader->close();
        delete m_XmlWriterTrajsHeader;
        m_XmlWriterTrajsHeader = NULL;
    }

    // Recopie du contenu du fichier temporaire dans le fichier des trajectoires dÃ©finitif.
...
void GMLDocTrafic::AddInstant(double dbInstant, double dbTimeStep, int nNbVeh)
{
    ...
    // Ecriture du dÃ©but de la pÃ©riode pour les feux tricolores
    if(m_XmlWriterTrafficLights)
    {
...
void GMLDocTrafic::AddPeriodeCapteur(double dbDeb, double dbFin, const std::string & nodeName)
{
 ...
 DocTrafic::PCAPTEUR GMLDocTrafic::AddInfoCapteur(const std::string & sIdCpt, const std::string & sTypeVeh, const std::string & sVitGlob, const std::string & sNbCumGlob, const std::string & sVitVoie, const std::string & sNbCumVoie, const std::string & sFlow, bool bExtractVehicleList)
 {
     if(m_XmlWriterSensors)
     {
         // rmq. Pour l'instant, on ne sort pas les rÃ©sultats par type de vÃ©hicule... Ã  voir si il faut le faire et si le format GML/OM peut le gÃ©rer
         if(sTypeVeh.empty())
         {
             m_XmlWriterSensors->writeStartElement(XS("gml:member"));
...
{
    // finalisation de la pÃ©riode du fichier des Ã©tats des feux tricolores
    if(m_XmlWriterTrafficLights)
    {
...
{
    // rÃ©cupÃ©ration des Ã©tats des feux tricolores
    ...
    // rÃ©cupÃ©ration des Ã©tats des capteurs
    if(m_XmlWriterSensors)
    {
    ...
    // rÃ©cupÃ©ration des nouveaux vÃ©hicules et nouvelles trajectoires
...
void GMLDocTrafic::AddTroncon(const std::string & strLibelle, Point * pPtAm, Point * pPtAv, int nVoie, const std::string & ssBrique, std::deque<Point*> &lstPtInterne, double dbLong, std::vector<double> dbLarg, double dbStockMax, double dbStockInitial, Tuyau * pTuyau)
{
    if(ssBrique.empty())
    {
        ...
        // Types de vÃ©hicules autorisÃ©s
        ...
        // lod0 - reprÃ©sentation filaire du tronÃ§on
        ...
        // lod1 - reprÃ©sentation surfacique du tronÃ§on
...
void GMLDocTrafic::AddDefCAF(CarrefourAFeuxEx * pCAF)
{
    ...
    // lod0 - reprÃ©sentation filaire du CAF
    m_XmlWriterNetwork->writeStartElement(XS("tran:lod0Network"));
    m_XmlWriterNetwork->writeStartElement(XS("gml:CompositeCurve"));
    ...
    // lod1 - reprÃ©sentation surfacique du CAF
    m_XmlWriterNetwork->writeStartElement(XS("tran:lod1MultiSurface"));
    m_XmlWriterNetwork->writeStartElement(XS("gml:MultiSurface"));
    m_XmlWriterNetwork->writeStartElement(XS("gml:surfaceMember"));
    m_XmlWriterNetwork->writeStartElement(XS("gml:Polygon"));
    ...
    // Ensemble des tuyaux liÃ©s au CAF
    ...
    // Calcul de l'angle entre l'extremitÃ© du tuyau qui touche le CAF et le barycentre du CAF
    ...
    for(size_t iTuyau = 0; iTuyau < lstTuyauxTri.size(); iTuyau++)
    {
        ...
        // Calcul des points extrÃªmes du tuyau
        std::deque<Point> pointsRight = pTuyau->GetLineString(largeurTotale/2.0);
        std::deque<Point> pointsLeft = pTuyau->GetLineString(-largeurTotale/2.0);
...
void GMLDocTrafic::AddDefGir(Giratoire * pGir)
{
    ...
    // lod0 - reprÃ©sentation filaire du Giratoire
    m_XmlWriterNetwork->writeStartElement(XS("tran:lod0Network"));
    m_XmlWriterNetwork->writeStartElement(XS("gml:CompositeCurve"));
    ...
    // lod1 - reprÃ©sentation surfacique du Giratoire
...
void GMLDocTrafic::AddDefCDF(ControleurDeFeux * pCDF)
{
    ...
    {            
        ...
        // un point pour chaque position diffÃ©rente en fonction de la voie amont et de la position des lignes de feux
        std::set<Point, LessPointPredicate> lstPoints;
        Connexion * pConnexion = cplES.pEntree->GetCnxAssAv();
        // Pour chaque voie du tuyau d'entrÃ©e
        for(size_t iUpLane = 0; iUpLane < cplES.pEntree->GetLstLanes().size(); iUpLane++)
        {
            if(pConnexion->m_mapMvtAutorises.find(cplES.pEntree->GetLstLanes()[iUpLane]) != pConnexion->m_mapMvtAutorises.end())
            {
                ...
                if(mapMoves.find(cplES.pSortie) != mapMoves.end())
                {
                    ...
                    // Pour chaque voie aval accessible, dÃ©termination de la position du feu et ajout Ã  l'ensemble
                    ...
                    {
                        // on dÃ©termine la voie sur laquelle se trouve le feu en tenant compte de la position de la ligne de feu
                        Voie * pVoie = NULL;
                        double dbPk = 0;
                        if(iter->second->m_dbLigneFeu > 0)
                        {
                            // Cas de la ligne de feu aprÃ¨s la fin du tronÃ§on amont
                            ...
                            {
                                // si la connexion n'est pas une brique, on positionne le feu sur la voie aval, en se ramenant Ã  la fin de la voie aval si ca dÃ©passe
                        ...
                        {
                            // Cas de la ligne de feu avant la fin du tronÃ§on amont
                            pVoie = cplES.pEntree->GetLstLanes()[iUpLane];

                            // on n'autorise pas la ligne de feux Ã  Ãªtre encore plus en arriÃ¨re que le dÃ©but de la voie amont
        ...
        // on fait ce test au cas oÃ¹ le scnÃ©ario soit mal dÃ©fini (un couple entrÃ©e sortie du CDF
        // ne correspond Ã  aucun mouvement autorisÃ© produit une liste de points vide. Du coup, autant ne pas sortir
        // ce feu qui ne sert Ã  rien)
        if(!lstPoints.empty())
        {
            ...
            // Ã©criture des positions pour le feu
...
void GMLDocTrafic::UpdateInstSortieVehicule(int nID, double dbInstSortie, const std::string & sSortie, double dstParcourue, const std::vector<Tuyau*> & itinerary, const std::map<std::string, std::string> & additionalAttributes)
{
}
        ...
        bool bTypeChgtVoie, TypeChgtVoie eTypeChgtVoie, bool bVoieCible, int nVoieCible, bool bPi, double dbPi, bool bPhi, double dbPhi, bool bRand, double dbRand, bool bDriven, const std::string & strDriveState, bool bDepassement, bool bRegimeFluide, const std::map<std::string, std::string> & additionalAttributes)
{
    ...
    // Attention au cas du premier instant pour le vÃ©hicule : one ne met que la position Ã  la fin du pas de temps(Ã  amÃ©liorer ?)
