    ...
    : LongitudinalSensor(strNom, pTuyau, startPos, endPos)
{
    ...
    data.dbTotalTravelledTime.resize(countNbVoies + 1);
    data.dbTotalTravelledDistance.resize(countNbVoies + 1);
    ...
    for(int i = 0; i < countNbVoies+1; i++)
    {
        for (size_t iTypeVeh = 0; iTypeVeh < pTuyau->GetReseau()->m_LstTypesVehicule.size(); iTypeVeh++)
        {
            ...
            data.dbTotalTravelledTime[i][pTypeVeh] = 0.0;
            data.dbTotalTravelledDistance[i][pTypeVeh] = 0.0;
        }
    }

	for (size_t iTypeVeh = 0; iTypeVeh < pTuyau->GetReseau()->m_LstTypesVehicule.size(); iTypeVeh++)
		data.nNumberOfRemovalVehicles[pTypeVeh] = 0;
}
...
void EdieSensor::CalculInfoCapteur(Reseau * pNetwork, double dbInstant, bool bNewPeriod, double dbInstNewPeriode, boost::shared_ptr<Vehicule> pVeh)
{
	...
	if (!pVeh->IsMeso())
	{
		...
		if (bFranchissement)
		{
			...
			if (dbDistance >= 0)
			{
                ...
                data.dbTotalTravelledDistance[numVoie][pVeh->GetType()] += dbDistance;
				// distance parcourue pour l' ensemble du tronçon
                data.dbTotalTravelledDistance[m_pTuyau->getNb_voies()][pVeh->GetType()] += dbDistance;
				...
				if (dbPasTemps > 0)
				{
                    ...
                    data.dbTotalTravelledTime[numVoie][pVeh->GetType()] += dbTemps;
					// temps passé sur toutes les voies :
                    data.dbTotalTravelledTime[m_pTuyau->getNb_voies()][pVeh->GetType()] += dbTemps;
				}
			}
		}
	}
	...
	if (!pVeh->GetLink(0))
	{
		if (pVeh->GetTrip()->GetFinalDestination()->GetInputPosition().IsLink())
		{
			...
			{
				data.nNumberOfRemovalVehicles[pVeh->GetType()] += 1;
			}
		}
	}
}
...
void EdieSensor::Write(double dbInstant, Reseau * pReseau, double dbPeriodeAgregation, double dbDebut, double dbFin, const std::deque<TraceDocTrafic* > & docTrafics, CSVOutputWriter * pCSVOutput)
{
    if(!docTrafics.empty())
    {
        ...
        double dbNbVoie = (double)data.dbTotalTravelledTime.size()-1;

        std::string strConcentration = "";
        std::string strDebit = "";

        double nbVoie = (double)data.dbTotalTravelledTime.size() - 1;
        for (size_t k = 0; k < data.dbTotalTravelledTime.size() - 1; k++)
        {
            strConcentration = strConcentration + SystemUtil::ToString(3, data.GetTravelledTimeForLane(k)/(dbPeriodeAgregation*dbLongueurCpt)) + " ";
            strDebit = strDebit + SystemUtil::ToString(3, data.GetTravelledDistanceForLane(k) / (dbPeriodeAgregation*dbLongueurCpt)) + " ";
        }
        ...
        for( itDocTraf = docTrafics.begin(); itDocTraf != docTrafics.end(); itDocTraf++ )
        {
            ...
            (*itDocTraf)->AddInfoCapteurEdie(m_strNom,   // identifiant capteur
                SystemUtil::ToString(3, data.GetTotalTravelledTime()/(dbPeriodeAgregation*dbLongueurCpt*nbVoie)),  // Concentration globale
                SystemUtil::ToString(3, data.GetTotalTravelledDistance()/(dbPeriodeAgregation*dbLongueurCpt*nbVoie)),// Débit global
...
{
    EdieSensorData * pResult = new EdieSensorData();
    pResult->dbTotalTravelledDistance = data.dbTotalTravelledDistance;
    pResult->dbTotalTravelledTime = data.dbTotalTravelledTime;
	pResult->nNumberOfRemovalVehicles = data.nNumberOfRemovalVehicles;
    return pResult;
}

void EdieSensor::Restore(Reseau * pNetwork, AbstractSensorSnapshot * backup)
{
    EdieSensorData * pSnapshot = (EdieSensorData*)backup;
    data.dbTotalTravelledDistance = pSnapshot->dbTotalTravelledDistance;
    data.dbTotalTravelledTime = pSnapshot->dbTotalTravelledTime;
	data.nNumberOfRemovalVehicles = pSnapshot->nNumberOfRemovalVehicles;
}
