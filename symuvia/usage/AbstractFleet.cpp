...
{
    // Action liÃ©es Ã  l'entrÃ©e du vÃ©hicule sur le TripNode
...
void AbstractFleet::OnVehicleActivated(boost::shared_ptr<Vehicule> pVeh, double dbInstant)
{
    ...
    // Positionnement du vÃ©hicule Ã  la sortie du TripNode correspondant
    ...
    {
        // l'insertion n'est plus Ã  faire
        pVeh->SetInstInsertion(dbInstant);

        // rmq. : une amÃ©lioration possible dans le cas des vÃ©hicules crÃ©Ã©s aux entrÃ©e serait de les intÃ©grer Ã©ventuellement
        // Ã  la liste des vÃ©hicules en attente Ã  l'entrÃ©e le cas Ã©chÃ©ant ...
    }
    ...
    // le premier arrÃªt est considÃ©rÃ© comme traitÃ© dÃ¨s le dÃ©part du bus
    ...
    // TODO - JORGE - pour l'instant, je ne touche pas au champ actuel agressif des vÃ©hicules qui sert Ã  la procÃ©dure d'agressivitÃ©. Faut-il fusionner les deux notions ?
	// AgressivitÃ©
	pVeh->SetAgressif(false);		// A reprendre, les autres flottes que la SymuVIaFleet ne sont pas agressifs pour l'instant (car reliÃ© aux entrÃ©es, ce qui n'est pas forcÃ©ment le cas dans les autres usages)

    // TODO - JORGE - a priori pas de raison de ne pas le gÃ©rer pour toute sles flottes ?
    pVeh->TirageJorge();
...
{
    // On enlÃ¨ve le vÃ©hicule de la liste des vÃ©hicules associÃ©s Ã  la flotte
    m_LstVehicles.erase(std::remove(m_LstVehicles.begin(), m_LstVehicles.end(), pVeh), m_LstVehicles.end());

    // On l'enlÃ¨ve du Trip correspondant le cas Ã©chÃ©ant
...
{
    // MÃ©thode du cas gÃ©nÃ©ral simple : un trip associÃ© Ã  un schedule. a priori utilisable pour tout un tas de cas d'usage,
    // donc factorisÃ© dans AbstractFleet
    
    std::vector<boost::shared_ptr<Vehicule> > result;
    
    // RÃ©cupÃ©ration du schedule
    std::map<Trip*,Schedule*>::const_iterator iter = m_MapSchedules.find(pTrip);
    if(iter != m_MapSchedules.end())
    {
        // rmq. : hypothÃ¨se : on ne gÃ¨re pas plusieurs crÃ©ations pour un pas de temps avec un Schedule.
...
boost::shared_ptr<Vehicule> AbstractFleet::CreateVehicle(double dbInstant, double dbTimeStep, Trip * pTrip, int vehId, ScheduleElement * pScheduleElement)
{
    ...
    {
        // rmq. : pas de gestion du numÃ©ro de voie dans le cas de AbstractFleet : une seule rÃ©partition des types de vÃ©hicules Ã  l'indice 0.
        TypeVehicule * pTypeVeh = iterRepType->second->CalculTypeNewVehicule(m_pNetwork, dbInstant, 0);

        // rmq. : on dÃ©marre le vÃ©hicule Ã  vitesse nulle dans le cas de la gestion gÃ©nÃ©rique des flottes... Ã  spÃ©cialiser si besoin
        ...
        // Ajout du vÃ©hicule Ã  la liste des vÃ©hicules gÃ©rÃ©s par le Trip
        ...
        // pour pouvoir utiliser dÃ¨s ici les paramÃ¨tres de chargement des vÃ©hicules qui sont crÃ©Ã©s Ã  un arrÃªt de bus par exemple,
...
bool AbstractFleet::AddTripNodeToTrip(Trip * pTrip, TripNode * pTripNode, size_t positionIndex, Vehicule * pVehicle)
{
    ...
    {
        // Si la position est trop grande, on la ramÃ¨ne au nombre de legs actuel (comme Ã§a le nouveau leg est ajoutÃ© Ã  la fin du Trip)
    ...
    if(pVehicle)
    {
        // Si la position du nouveau leg est la position du leg courant, il faut adapter les itinÃ©raires en cours de parcours ...
        ...
        {
            // dans ce cas, on calcule l'itinÃ©raire Ã  partir de la position courante du vÃ©hicule
        ...
        if(paths.size() == 1)
        {
            ...
            // Recalcul de l'itinÃ©raire du leg suivant Ã  partir de la nouvelle origine s'il y en a un (de leg suivant)
            if(positionIndex < pTrip->GetLegs().size())
            {
    ...
    else
    {
        ...
        pTripLeg->SetDestination(pTripNode);
        pTrip->InsertTripLeg(pTripLeg, positionIndex);
        // rmq. : on ne recalcule pas d'itinÃ©raire ici car les itinÃ©raires sont calculÃ©s Ã  l'activation des vÃ©hicules
        // (tel que codÃ© au moment de la rÃ©alisation de cette mÃ©thode en tout cas)
    }
...
bool AbstractFleet::RemoveTripLegFromTrip(Trip * pTrip, size_t positionIndex, Vehicule * pVehicle)
{
    ...
    if(pVehicle)
    {
        // Recalcul de l'itinÃ©raire du leg suivant le leg supprimÃ© (son origine change), avec gestion du cas particulier oÃ¹ le vÃ©hicule est sur le leg supprimÃ© !
        ...
        if(bOk)
        {
            if(bNextLegChanged)
            {
                // On modifie le TripLeg courant pour aller vers la destination suivante avec l'itinÃ©raire calculÃ©,
            ...
            else
            {
                ...
                // puisqu'on ne permet de supprimer que des points de livraison pour l'instant. Peut-Ãªtre faudra-t-il faire plus de choses ici
                // si on y passe un jour (quid du vÃ©hicule qui n'a plus rien Ã  faire ? ou le supprime ?)
...
void AbstractFleet::Restore(Reseau * pNetwork, AbstractFleetSnapshot * backup)
{
    ...
    for(size_t i = 0; i < backup->lstVehicles.size(); i++)
    {
        ...
        if (!pVehToRestore)
        {
            // il s'agit d'un vÃ©hicule en attente Ã  une entrÃ©e :
