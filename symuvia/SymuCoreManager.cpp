...
{
    // rmq. : les macrotypes sont dÃ©truits avec le Graph, donc on ne les dÃ©truit pas ici
...
void SymuCoreManager::Initialize()
{
    ...
    // Nettoyage de l'ancienne dÃ©finition du rÃ©seau
    if (GraphExists())
    {
        delete m_pGraph;
    }
    ...
    ClearSubPopulations();
    // CrÃ©ation d'une population pour chaque type de vÃ©hicule !
    ...
    // CrÃ©ation de la sous population pour les calculs d'itinÃ©raires en TEC uniquement
        ...
        for (size_t iPattern = 0; iPattern < pLink->getListPatterns().size(); iPattern++)
        {
            ...
            if (pSymuViaDrivingPattern)
            {
                ...
                if (pTronconPattern)
                {
                    ...
                    // rmq. : on laisse du coup Ã  0 le terme beta associÃ© Ã  la classe fonctionnelle pour les autres types de patterns... il faudra se poser
                    // la question quand on fera du multimodal et du A* de si on laisse comme Ã§a : en l'Ã©tat, on va pÃ©naliser la route sur les TECs.
    ...
    // Positionnement de la classe fonctionnelle des tronÃ§ons routier pour lesquels elle n'est pas renseignÃ©e :
    ...
    for (size_t iNode = 0; iNode < m_pGraph->GetListNodes().size(); iNode++)
    {
        ...
        for (iter = pNode->getMapPenalties().begin(); iter != pNode->getMapPenalties().end(); ++iter)
        {
            for (std::map<SymuCore::Pattern*, SymuCore::AbstractPenalty*, SymuCore::ComparePattern >::iterator iterDown = iter->second.begin(); iterDown != iter->second.end(); ++iterDown)
            {
                ...
                // Pour filtrer les NullPenalty qui ne nous intÃ©ressent pas ici :
                if (pPenalty)
                {
    ...
    // Calcul des couts et pÃ©nalisations
    ...
    for (size_t iLink = 0; iLink < m_pGraph->GetListLinks().size(); iLink++)
    {
        ...
        for (size_t iPattern = 0; iPattern < pLink->getListPatterns().size(); iPattern++)
        {
            ...
            else if (m_pNetwork->IsWithPublicTransportGraph())
            {
                ...
                // rien Ã  faire pour la marche Ã  pieds : cout fixe dÃ©jÃ  calculÃ© Ã  l'initialisation
            }
        }
    }
...
void SymuCoreManager::CheckGraph(bool bIsRecursiveCall)
{
    ...
    else if(m_bInvalid)
    {
        ...
        // Dans ce cas, on demande Ã  calculer des itinÃ©raires alors qu'une modification du rÃ©seau a eu lieu, impactant le graphe mais ne dÃ©clenchant pas automatiquement de recalcul de l'affectation (modification
        // des mouvements autorisÃ©s). On recalcul le nouveau graphe en conservant les couts actuels des diffÃ©rents liens avant de procÃ©der au calcul d'itinÃ©raire.

        // On note les couts dÃ©finis dans le graphe prÃ©cÃ©dent
        ...
        // On restaure les couts notÃ©s dans le nouveau graphe. Les autres couts sont les couts calculÃ©s lors du Initialize.
...
void SymuCoreManager::ComputePaths(Connexion* pOrigine, TripNode* pDestination, TypeVehicule * pTypeVeh, double dbInstant, int nbShortestPaths, Tuyau * pTuyauAmont,
    ...
    std::map<std::vector<Tuyau*>, double> & MapFilteredItis, bool bRecursiveCall)
{
    // convenience method pour ne pas avoir Ã  changer les appels de partout dans SymuVia :
    if (pTuyauAmont == NULL)
    {
        CheckGraph(bRecursiveCall);
    ...
    else
    {
        ...
        // Il faut retirer le tuyau amont de l'itinÃ©raire rÃ©sultat...
        for (size_t iPath = 0; iPath < paths.size(); iPath++)
        {
            ...
            // rmq. : pour Ãªtre rigoureux, il faudrait modifier aussi les itinÃ©raires de MapFilteredItis et recalculer les couts
            // mais ils ne sont pas utilisÃ©s en cas d'appel Ã  cette fonction avec un tuyau amont imposÃ©, donc...
        }
    }
}
    ...
    std::vector<std::vector<PathResult> > & paths, std::vector<std::map<std::vector<Tuyau*>, double> > & MapFilteredItis)
{
    ...
    // Pour gestion du calcul rapide si plusieurs origines diffÃ©rentes :
    bool bMultipleOrigins = lstOrigins.size() > 1;
    bool bMultipleDestinations = lstDestinations.size() > 1;

    assert(!(bMultipleOrigins && bMultipleDestinations)); // on ne sait pas faire, ou alors il faut dÃ©composer et pas besoin pour l'instant car on ne fait pas ce type d'appel

    bool bMultiTarget = bMultipleOrigins || bMultipleDestinations;

    // mode multiorigine/destination utilisÃ© uniquement pour du test de chemin possible, donc avec nbShortestPaths = 1
    ...
    if (bMultiTarget)
    {
        ...
        // rmq. : l'autre option serait de faire du A* mais en dÃ©composant en 1 origine <-> 1 destination...
    ...
    {
        // rÃ©cupÃ©ration des paramÃ¨tres de commonality filter et chemins prÃ©dÃ©finis pour le couple OD et pour l'instant considÃ©rÃ© :
        ...
        // remarque : pour faire comme avant, on ne cherche pas de paramÃ¨tres prÃ©dÃ©finis si on a des liens origine ou destination.
        // Autre option : on pourrait ne pas faire ce test, mais aprÃ¨s on a des assert qui pÃ¨tent car on s'attend pas Ã  avoir plus d'un itinÃ©raire quand on n'en demande qu'un...
        // Autre option : modifier le module de calcul d'itinÃ©raire pour ne renvoyer qu'un itinÃ©raire mÃªme si on a en a plusieurs prÃ©dÃ©finis, ca semblerait logique aussi ...
                    ...
                    if (pMatDest)
                    {
                        ...
                        for (size_t iDest = 0; iDest < pMatDest->MatOrigDest.size(); iDest++)
                        {
                            ...
                            if (pVectDest->pDest == iterDestination->second)
                            {
                                ...
                                // gestion des itinÃ©raires prÃ©dÃ©finis :
        ...
        // Gestion des itinÃ©raires filtrÃ©s :
    ...
    for (size_t iOrigin = 0; iOrigin < lstOriginsForSymuCore.size(); iOrigin++)
    {
        ...
        for (size_t iDestination = 0; iDestination < lstDestinationsForSymuCore.size(); iDestination++)
        {
            ...
            for (size_t i = 0; i < symucorePaths.size(); i++)
            {
                ...
                // point de jonction pour les itinÃ©raires vides (zones collÃ©es)
                if (path.links.empty())
                {
                    ...
                    {
                        // la suite ne fonctionne que si on est dans le cas Zone -> Zone (avec deux patterns dans l'itinÃ©raire du coup).
                        // Si un seul pattern, on est dans le cas Zone -> extremitÃ© ponctuelle ou extremitÃ© ponctuelle -> Zone, cas pour lequel
                        // le point de jonction est inutile (c'est forcÃ©ment l'extremitÃ© ponctuelle).
    ...
    std::vector<std::vector<std::pair<double, std::vector<std::string> > > > & paths)
{
    ...
    if (myOrigin.getNode())
    {
        ...
        // On rÃ©injecte les destinations inatteignables (noeud null) dans path :
    ...
    std::vector<std::pair<double, std::vector<std::string> > > & paths)
{
    ...
    {
        // PÃ©nalisation des patterns des lignes amont et aval :
        ...
        if (pNextLine)
        {
            ...
            // si on trouve pas la ligne, il faut pÃ©naliser les patterns de la ligne Ã  pÃ©naliser aprÃ¨s marche Ã  pieds vers cette ligne :
    ...
    std::vector<std::vector<std::pair<double, std::vector<std::string> > > > & paths)
{
    ...
    // mode multidestination utilisÃ© uniquement pour du test de chemin possible, donc avec nbShortestPaths = 1
    ...
    for (size_t iDest = 0; iDest < lstDestinationsForSymuCore.size(); iDest++)
    {
        ...
        for (size_t i = 0; i < symucorePaths.size(); i++)
        {
            ...
            // liste des Ã©tapes du chemin sous la forme : nom arret nom ligne nom arrret nom ligne nom arret.
