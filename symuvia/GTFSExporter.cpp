...
bool GTFSExporter::writeStops(const std::string & outDir, Reseau * pNetwork)
{
    ...
    // Le format GTFS attend des coordonnées en lat long WGS84.
    // On suppose ici que le système d'entrée est Lambert93 (2154), mais il fvaudrait mieux se baser sur le champ EPSGinput du fichier XML d'entrée ? le problème est qu'il
    // n'est souvent pas renseigné...
	...
	if(!pCoordTransf)
    {
        std::cerr << "Unable to create the requested OGR transformation..." << std::endl;
        ofs.close();
	    return false;
    }
...
void GTFSExporter::writeCalendar(const std::string & outDir)
{
    ...
    // rmq. : les champs date de début et de fin sont optionnels dans le standard, mais tempus veut des valeurs. Du coup on met une plage bidon très large.
    // Peut-être faudrait-il mettre uniquement la date de la simulation (le problème est que cette date est facultative dans le format SymuVia...) ?
    ofs << SYMUVIA_GTFS_SERVICE << ",1,1,1,1,1,1,1,20000101,20200101" << std::endl;
    ofs.close();
}


// ecriture du fichier calendar_dates.txt (a priori optionnel mais le loader tempus le considère obligatoire ...)
...
// écriture du fichier routes.txt
...
// écriture du fichier stop_times.txt
void GTFSExporter::writeStopTimes(const std::string & outDir, Reseau * pNetwork)
{
    ...
    for(size_t iLine = 0; iLine < lstLines.size(); iLine++)
    {
        ...
        // Calcul du temps de parcours à vide entre chaque arrêt pour avoir une estimation des horaires de bus à tous les arrêts de la ligne
        ...
        // Constitution de la liste des instants de départ
        ...
        for(size_t iElem = 0; iElem < pLineSchedule->GetLstElements().size(); iElem++)
        {
            ...
            {
                // On ignore les fréquences (mais on garde la première pour ajouter l'instant de démarrage s'il n'y a que des fréquences dans le calendrier)
        ...
        // Si que des fréquences, on met l'instant de démarrage de la première fréquence
        ...
        for(size_t iDeparture = 0; iDeparture < lstDepartureTimes.size(); iDeparture++)
        {
            ...
            // Heure de départ
            // Remarque : on ignore cette ligne s'il s'agit d'une heure de départ à une entrée et pas au premier arrêt (puisque la colonne stop_id doit correspondre à un nom d'arrêt)
            ...
            for(size_t iLeg = 0; iLeg < pLine->GetLegs().size(); iLeg++)
            {
                ...
                if(pStop)
                {
                    ...
                    // Si une durée d'arrêt est définie en particulier pour ce départ, on l'utilise à la place
                    ...
                    // Prise en compte de la durée de déplacement
                    STimeSpan ts2(0, 0, 0, (int)(dbDriveTimesBetweenStops[iLeg]+0.5));
                    STimeSpan::RegulariseSomme(ts2);
                    stopTime = tDepartureTime + ts2;

                    // Dans le cas particulier du dernier arrêt; on ne compte pas le temps d'arrêt
                    ...
                    {
                        // Prise en compte de la durée de l'arrêt
...
// écriture du fichier frequencies.txt
