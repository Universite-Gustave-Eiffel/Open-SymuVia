...
// Teste si un tronÃ§on est un tronÃ§on amont de la connexion
...
// Teste si un tronÃ§on est un tronÃ§on aval de la connexion
...
// Teste si un mouvement est autorisÃ©
...
// Accesseur (ajoutÃ© pour accÃ¨s depuis python)
...
//----------------------------------------------------------------
// Fonction   :  Renvoie l'indice de l'Ã©lÃ©ment amont passÃ© en paramÃ¨tre
...
//----------------------------------------------------------------
// Fonction   :  Renvoie l'indice de l'Ã©lÃ©ment aval passÃ© en paramÃ¨tre
// Version du :  07/02 /2005
// Historique :  07/02/2005 (C.BÃ©carie - Tinea)
//              CrÃ©ation
//================================================================
...
//----------------------------------------------------------------
// Fonction  : Retourne la rÃ©partition du flux de l'Ã©lÃ©ment amont
//             vers l'Ã©lÃ©ment aval passÃ© en paramÃ¨tre Ã  un instant
//             donnÃ©
// Version du: 29/09/2006
// Historique: 29/09/2006 (C.BÃ©carie - Tinea)
//              Si l'instant simulÃ© est supÃ©rieur Ã  la somme des durÃ©es
//              des rÃ©partitions dÃ©finies, on retourne la derniÃ¨re
//              rÃ©partition
//             11/01/2005 (C.BÃ©carie - Tinea)
//              CrÃ©ation
//================================================================
(
    TypeVehicule * pTypeVeh,// Type de vÃ©hicule
...
bool Connexion::LoadMouvementsAutorises(XERCES_CPP_NAMESPACE::DOMNode *xmlNodeMvtsAutorises, Logger * pLogger, bool bInsertion, std::string strTAvPrincipal, int nbVoiesEch)
{
	...
	for(XMLSize_t i=0; i<counti; i++)
	{
		...
		// Chargement du libellÃ© du tronÃ§on amont
		...
		for(XMLSize_t j=0; j< count; j++)
		{
			...
			// Chargement du libellÃ© du tronÃ§on aval
			::GetXmlAttributeValue( xmlNodeMvtAval, "id_troncon_aval", sId, pLogger);

            // Cas particulier de l'insertion
            // EVOLUTION nÂ°70 : si plusieurs tronÃ§on aval cas de "l'Ã©changeur", il faut incrÃ©menter le numÃ©ro de voie
            ...
            // EVOLUTION nÂ°70 - dÃ©calage des numÃ©ros de voie du tronÃ§on principal (qui n'est plus le tronÃ§n le plus Ã  droite)
            if(bInsertion)
            {
                nVAv+=nDecalVoie;
            }
            ...
            // Chargement des types de vÃ©hicules exclus
            std::string strTmp;
		...
		// VÃ©rification que la somme des taux d'utilisation pour un tronÃ§on aval soit Ã©gale Ã  1 (ou 0)
		std::map<Voie*, std::map<Tuyau*, std::map<int, boost::shared_ptr<MouvementsSortie> >, LessPtr<Tuyau> >, LessPtr<Voie> >::iterator itVAm = this->m_mapMvtAutorises.find( pVAm );		
		if( itVAm != this->m_mapMvtAutorises.end() )
		{			
			...
			for( itTAv = m_mapMvtAutorises.find( pVAm )->second.begin(); itTAv != m_mapMvtAutorises.find( pVAm )->second.end(); itTAv++)
			{
                ...
                // Si on a plusieurs voies aval et que tous els taux vallent 1 (par dÃ©faut), on remplace par une Ã©quirÃ©partition des taux des voies
...
// Chargement des donnÃ©es d'affectation prÃ©dÃ©finies
bool Connexion::LoadAffectation(DOMNode *xmlNodeAffectation, bool bAllType, Logger *pChargement)
{
	...
	if( !bAllType )		// Affectation par type de vÃ©hicule
	{
	...
	else	// Affectation commune Ã  tous les types de vÃ©hicule
	{
...
bool Connexion::LoadAffectationEx(DOMNode *xmlNodeAffectations, TypeVehicule *pTV, Logger *pChargement)
{
	...
	for(XMLSize_t j=0; j<countj; j++)
	{
		...
		// Chargement de la durÃ©e de la variante		
		...
		// Boucle sur les couples entrÃ©e/destination
		...
		for(XMLSize_t k=0; k<countk; k++)
		{
			...
			::GetXmlAttributeValue( xmlNodeCoupleEntreeDestination, "id_troncon_amont", sEntree, pChargement);	// TronÃ§on amont
			...
			for(XMLSize_t l=0; l<countl; l++)
			{
				...
				::GetXmlAttributeValue( xmlNodeSortieConnexion, "id_troncon_aval", sSortie, pChargement);	// TronÃ§on aval
			...
			// VÃ©rification somme des coefficients d'affectation pour le couple entrÃ©e/destination = 1
    ...
    // vÃ©rification des plages temporelles (couverture de toute la simu)
...
{
    // Recherche en prioritÃ© des plages temporelles dÃ©finies
    ...
    if(!plageFound) 
    {
	    ...
	    if( itVTAffectation == dqVTAffectation.end())			// si variante temporelle non dÃ©finie, on prend la derniÃ¨re
		    mapCoupleCoeffs = dqVTAffectation.back().second;
	    else
		    mapCoupleCoeffs = itVTAffectation->second;
    }

	// Recherche du couple entrÃ©e/destination
...
// MAJ des coefficients d'affectation Ã  partir des coÃ»ts (Ã  l'aide d'un logit)
...
{	
	...
	// Remise Ã  zÃ©ro des coefficients d'affectation
	if( m_mapCoeffAffectation.find( pTV ) != m_mapCoeffAffectation.end() )
		m_mapCoeffAffectation.find(pTV)->second.clear();

    // Suppression des variantes temporelles dÃ©finies par plages (de toute faÃ§on pas utilisÃ©es car Affectation calculÃ©e automatiquement ...)
    if( m_mapCoeffAffectationPT.find( pTV ) != m_mapCoeffAffectationPT.end() )
		m_mapCoeffAffectationPT.find(pTV)->second.clear();
		
	if( m_mapCoutAffectation.find( pTV) != m_mapCoutAffectation.end() )
	{
		// Boucle sur les couples entree/destination avec coÃ»t non nul
		...
		for( itCpl = m_mapCoutAffectation.find( pTV)->second.begin(); itCpl != m_mapCoutAffectation.find( pTV)->second.end(); itCpl++)
		{
			...
			double dbSum = 0;
			mapCoeff.clear();
			// Boucle sur les tronÃ§ons avals possible

			// 
			if( dbTetaLogit > 10 )		// Traitement d'un teta grand (sinon passage Ã  la limite mal traitÃ© numÃ©riquement)
				dbTetaLogit = 10;			
			
			for( itTCout = (*itCpl).second.begin(); itTCout != (*itCpl).second.end(); itTCout++)
			{
				dbSum += exp( - dbTetaLogit * (*itTCout).second );	// Calcul de la somme exponentielle des coÃ»t
			}
			// Calcul du rapport logit pour chaque tronÃ§on aval possible
			for( itTCout = (*itCpl).second.begin(); itTCout != (*itCpl).second.end(); itTCout++)
			{
...
// IncrÃ©mentation du nombre de vÃ©hicules
...
{	
	...
	if( mapTSAss.find( pairTS ) != mapTSAss.end() )
	{
		...
		{
			// Recherche de l'itinÃ©raire ou du tronÃ§on aval correspondant
...
// Calcul le barycentre de la connexion Ã  partir des coordonnÃ©es des tronÃ§ons amont et aval
...
{
    // On choisit ici de dÃ©finir un marginal de 0 pour les connexions ponctuelles plutÃ´t que NaN
    return 0;
}
#endif // USE_SYMUCORE
...
bool MouvementsSortie::IsTypeExclu(TypeVehicule* pTV, SousTypeVehicule * pSousType)
{
    ...
    if(pSousType)
    {
        // Dans ce cas, on regarde s'il y a des briques qui rÃ©gulent ce mouvement.
        // Si oui, on renvoie vrai si le type est interdit par au moins une brique que le vÃ©hicule respecte, et faux sinon
        // Si non, on renvoie la valeur par dÃ©faut bDefaultResult
    ...
    else
    {
        ...
        if(!bResult)
        {
            // rmq. : la rÃ©gulation de zone du tonkin ne fonctionnait plus (rÃ©gression suite Ã  la mise en place de la notion
            // de respect de la consigne (voir les sous types de vÃ©hicules) : on regarde donc, dans le cas oÃ¹
            // aucun sous type n'est dÃ©fini, si au moins une brique de rÃ©gulation interdit le mouvement,
            // en esperant ne pas introduire de rÃ©gression dans la fonctionnalitÃ© de taux de respect de la consigne...
            map<RegulationBrique*, vector<TypeVehicule*> >::iterator iter;
            for(iter = m_LstForbiddenVehicles.begin(); iter != m_LstForbiddenVehicles.end(); ++iter)
            {
                if(iter->first != NULL)
...
void MouvementsSortie::AddTypeExclu(TypeVehicule* typeVehicule)
{
    ...
    // Le graphe d'affectation a changÃ© ...
