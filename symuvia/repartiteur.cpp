...
// constructeur complet du rÃ©partiteur
...
// crÃ©ation automatique des mouvements autorisÃ©s pour un rÃ©partiteur gÃ©nÃ©rÃ© automatiquement pour l'insertion
...
{
    // EVOLUTION nÂ°70 : cas de l'Ã©changeur : on peut avoir plusieurs tronÃ§ons aval
    ...
    /// EVOLUTION nÂ°70 - cas de l'Ã©changeur avec plus d'un tuyau aval.
    for(size_t iTuyAvalSec = 0; iTuyAvalSec < m_LstTuyAv.size(); iTuyAvalSec++)
    {
...
)
{
		    ...
		    for(itSorties = (*itMvtAutorises).second.begin(); itSorties != (*itMvtAutorises).second.end(); itSorties++)            
            {
			    ...
			    for(itVoiesSortie = (*itSorties).second.begin(); itVoiesSortie != (*itSorties).second.end(); itVoiesSortie++)        
			    {
                    ...
                    if(itVoiesSortie->second->m_dbLigneFeu > 0)
                    {
                        ...
                        *pOfLog << Logger::Info; // rebascule en mode INFO pour ne pas avoir Ã  reprendre tous les appels aux log en prÃ©cisant que c'est des INFO. Ã  supprimer si on reprend tous les appels au log.
                    }
    ...
    // Calcul des angles des mouvements, pour dÃ©termination des tourne Ã  droite ou gauche
    Tuyau * pUpstream, * pDownstream;
    for(size_t i = 0; i < m_LstTuyAm.size(); i++)
    {
        pUpstream = m_LstTuyAm[i];

        // RÃ©cupÃ©ration du segment aval du tuyau amont
        ...
        else
        {
            ...
            for(int k = (int)pUpstream->GetLstPtsInternes().size()-1; k >= -1 ; k--)
            {
                ...
                else
                {
                    ...
                    {
                        // On continue tant qu'on n'a pas trouvÃ© un point diffÃ©rent du premier
                        break;
                    }
                }
            }
        ...
        for(size_t j = 0; j < m_LstTuyAv.size(); j++)
        {
            ...
            {
                // On ramÃ¨ne le segment du tuyau aval sur le point amont du segment amont
                // pour avoir les trois points nÃ©cessaires au calcul de l'angle qui nous intÃ©resse
...
//--------------------------------------------------------------------------
// Fonctions relatives Ã  la simulation des rÃ©partiteurs
//--------------------------------------------------------------------------
...
// ------------------------------------
// ------------------------------------
// Simulation effective des rÃ©partiteur
...
)
{
    ...
    double **     Di; // Tableau des Demandes de chaque entrÃ©e
    double **     Dj; // Tableau des Demandes de chaque sortie
    double **     Oj; // Tableau des Offres de chaque sortie
    double **     Qj; // Tableau des DÃ©bits de chaque sortie
    double **     Qi; // Tableau des DÃ©bits de chaque entrÃ©e
    double ****   Dij;  // Tableau Ã  2 dim demande de i vers j
    double ****   Qij; // Tableau Ã  2 dim dÃ©bit de i vers j
    double **     Di_feux; // Tableau des demandes de chaque entrÃ©e en tenant compte des feux
    double **     Dj_feux; // Tableau des demandes de chaque sortie en tenant compte des feux

    // Cette procÃ©dure doit Ãªtre appelÃ©e uniquement dans le cas d'un rÃ©partiteur de flux
    //if( !IsFlux() )
      //  return;
    ...
    // On rÃ©cupÃ¨re la demande des tronÃ§ons se situant en amont du rÃ©partiteur
    ...
    // On rÃ©cupÃ¨re l'offre des tronÃ§ons se situant en aval du rÃ©partiteur
    // On la stocke dans O	
	i = 0;
    ...
    // et de l'Ã©tat des feux pour rÃ©partir la demande de chaque entrÃ©e
    // On stocke le tout dans Dij
    // On en profite pour dÃ©terminer pour chaque entrÃ©e la demande i qui a la potentialitÃ© de s'Ã©couler (la demande
    // ne peut s'Ã©couler si le feu de i vers j est au rouge.
    // On stocke la demande pouvant potentiellement s'Ã©couler dans Di_feux

	ControleurDeFeux *pCDF = GetCtrlDeFeux();
	double dbPropVertOrange;

    // rmq. : les coefficients directionnels sont Ã  prÃ©sent dÃ©finis par type de vÃ©hicule : on utilise la rÃ©partition du type de vÃ©hicule principal ici (le premier de la liste par convention)
    ...
    // Calcul des dÃ©bits de sortie. Ce calcul s'effectue en dÃ©terminant tout d'abord
    // la demande voulant sortir Ã  chaque sortie (Dj) comme la somme des demandes Dij pour toutes
    // les entrÃ©es i. Cette demande est comparÃ©e Ã  l'offre de la sortie (Oj). Le dÃ©bit (Qj) est le
    // minimum des deux. Si la demande prÃ©domine la sortie est fluide (sortie_j_fluide = true) sinon
    // la sortie est congestionnÃ©e.
	...
	for(itT = m_LstTuyAv.begin(); itT != m_LstTuyAv.end(); itT++)    
    {
        for(l=0; l< m_nNbMaxVoies; l++)
        {
            ...
            {
                Qj[j][l]=Dj[j][l]; // Cas fluide le dÃ©bit est Ã©gal Ã  la demande
                sortie_j_fluide[j][l]=true;
            }
            else // Sortie congestionnÃ©e
            {
                Qj[j][l]=Oj[j][l];
    ...
    // Calcul des dÃ©bits par origine/destination. Le dÃ©bit Qij est calculÃ© Ã  partir du dÃ©bit
    // sortant de j (Qj) au prorata de la demande de i vers j par rapport Ã  la demande pour la sortie j
	i = 0;
	for(itT = m_LstTuyAm.begin(); itT != m_LstTuyAm.end(); itT++)
	{
        for(k=0; k<m_nNbMaxVoies;k++)
            ...
            for(j=0;j< (int) m_LstTuyAv.size();j++)
                for(l=0; l<m_nNbMaxVoies;l++)
                {
                    if(Dj[j][l]==0) // La demande vers la sortie j est nulle
                        Qij[i][k][j][l]=0; // Tous les dÃ©bits vers j sont donc nuls
                    else
    ...
    // Calcul des dÃ©bits pour chaque entrÃ©e (Qi) comme la somme des Qij pour tous les j
    // On dÃ©termine ensuite si l'entrÃ©e est fluide (Qi=Di) ou si elle est congestionnÃ©e (Qi<Di)
    // NB: si la demande pouvant potentiellement s'Ã©couler est nulle (Di_Feux=0) alors que la demande
    // est non nulle (cas d'un feu rouge qui bloque toute la voie) alors l'entrÃ©e est considÃ©rÃ©e comme
    // congestionnÃ©e
    // A vÃ©rifier : Ce dernier calcul est peut-Ãªtre redondant
    for(i=0;i<  (int) m_LstTuyAm.size();i++)
        for(k=0; k<m_nNbMaxVoies;k++)
        {
            Qi[i][k]=0;
    ...
    // Affectation des des dÃ©bits entrant Qi sortant Qj dans les tableaux les stockant

    for(i=0;i<  (int) m_LstTuyAm.size();i++)
    {
        for(k=0; k<m_nNbMaxVoies; k++)
            debit_entree[i][k]=Qi[i][k];     // le dÃ©bit est affectÃ©
    }

    for(j=0;j<  (int) m_LstTuyAv.size();j++)
    {
        for(l=0; l<m_nNbMaxVoies; l++)
            debit_sortie[j][l] = Qj[j][l];      // le dÃ©bit est affectÃ©
    }
...
//--------------------------------------------------------------------------
// Fonctions liÃ©es Ã  la gestion des autobus
//--------------------------------------------------------------------------


//---------------------------------------------------------------------------
// Fonctions de renvoi l'Ã©tat du rÃ©partiteur
//--------------------------------------------------------------------------

// renvoi du dÃ©bit de sortie vers le troncon n
...
// renvoi du dÃ©bit d'entrÃ©e provenant du troncon n
...
// renvoi l'Ã©tat de l'entrÃ©e avec le troncon n
...
// renvoi l'Ã©tat de la sortie vers le troncon n
...
// renvoi la vitesse maximal autorisÃ©e vers le tronÃ§on n
double Repartiteur::getVitesseMaxEntree(std::string n, int nVoie)
{
	//renvoie la vitesse maximale en entrÃ©e concernant le tronÃ§on demandeur
	std::deque<Tuyau*>::iterator itT;
	int i = 0;
...
// Calcul le rayon du rÃ©partiteur
...
//---------------------------------------------------------------------------
// Fonctions de renvoi des paramÃ¨tres du rÃ©partiteur
//--------------------------------------------------------------------------

// renvoi le nombre de tronÃ§ons amont
int Repartiteur::getNbAmont(void)
        { return (int) m_LstTuyAm.size(); }

//---------------------------------------------------------------------------
// Fonctions de modifications des paramÃ¨tres du rÃ©partieur
//--------------------------------------------------------------------------

// Affecte le type du rÃ©partiteur
...
double Repartiteur::GetVitesseMaxSortie(int n, int nVoie, double dbTime)
{
    ...
    // Initialisation de la vitesse max Ã  la vitesse libre
	...
	{		
		...
		{
			// La vitesse max est calculÃ©e comme le minimum des vitesses max des voies en amont du rÃ©partiteur
			for(k=0; k<m_nNbMaxVoies ;k++)
			{
				// La vitesse max de la voie en amont est considÃ©rÃ©e ssi la rÃ©partition vers la voie en aval traitÃ©e est non nulle            
...
// Remarque   :  Cette vitesse est dÃ©terminÃ©e comme le minimum des
//               vitesses maximales au niveau des diffÃ©rentes entrÃ©es
//               pour lesquelles le feu est vert
// Version du :  06/01/2005
// Historique :  06/01/2005 (C.BÃ©carie - Tinea)
//              CrÃ©ation
//================================================================
(
    double  dbTime
...
//----------------------------------------------------------------
// Fonction   :  Met Ã  jour les SAS entrÃ©e des voies aval d'un
//               rÃ©partiteur Ã  partir des SAS sortie des voies amont
// Remarque   :  Cette mise Ã  jour peut sembler inutile car les SAS
//               aprÃ¨s le calcul de dÃ©bit doivent toujours Ãªtre complÃ©mentaire...
//               Or dans le cas de la crÃ©ation d'un vÃ©hicule sur une voie amont vide,
//               la demande est modifiÃ©e et cette modification doit Ãªtre pris
//               en compte dans les tuyaux en aval. Ceci n'est pas le cas pour l'instant
//               dans l'algorithme d'oÃ¹ cette verrue...
//               (doit aussi rÃ©soudre le pb lorsque au cours du mÃ«me pas de temps
//               plusieurs vÃ©hicules sont crÃ©Ã©s sur la mÃªme voie...)
...
//----------------------------------------------------------------
// Fonction   : IncrÃ©mente le compteur du nombre de vÃ©hicule
//              traversant le rÃ©partiteur pour l'origine et la
//              destination passÃ©es en paramÃ¨tre. Chaque cycle de
//              feu possÃ¨de son propre compteur, ce qui permet de
//              connaÃ®tre le nombre de vÃ©hicule traversant Ã  chaque
//              cycle.
// Version du :  24/11/2006
// Historique :  24/11/2006 (C.BÃ©carie - Tinea)
//              CrÃ©ation
//================================================================
(
    TypeVehicule* pTypeVeh, // Type de vÃ©hicule (AjoutÃ© par cohÃ©rence avec l'ajout du type de vÃ©hicule pour les coefficients directionnels, mais fonction inutilisÃ©e de toute faÃ§on.
    int nAmont,             // NumÃ©ro du tronÃ§on amont
    int nVoieAmont,         // NumÃ©ro de voie du tronÃ§on amont
    int nAval,              // NumÃ©ro du tronÃ§on aval
    int nVoieAval,          // NumÃ©ro de voie du tronÃ§on aval
    double dbTime           // Instant de traversÃ©e du rÃ©partiteur
)

{
    ...
    // Recherche de la rÃ©partition active
...
//----------------------------------------------------------------
// Fonction   :  Retourne le nombre de vÃ©hicule comptabilisÃ©
// Version du :  27/11/2006
// Historique :  27/11/2006 (C.BÃ©carie - Tinea)
//              CrÃ©ation
//================================================================
(
    TypeVehicule* pTypeVeh, // Type de vÃ©hicule (AjoutÃ© par cohÃ©rence avec l'ajout du type de vÃ©hicule pour les coefficients directionnels, mais fonction inutilisÃ©e de toute faÃ§on.
...
{
    double dbResult = 0;
    // Si le tuyau amont a des temps mesurÃ©s, pas la peine de pÃ©naliser davantage (ca ferait doublon)
